...
const WORKER = resolve(HERE, "p11-f7-zero-unused-worker.mts");

const CONTROL_SHA = "281757be11fe3e2073bac9d7d7bf2cc24904dba6";
...
if (controlSha !== CONTROL_SHA) {
  throw new Error(
    `P11-F7 control must be ${CONTROL_SHA}, received ${controlSha}`,
  );
}

if (candidateSha !== CANDIDATE_SHA) {
  throw new Error(
    `P11-F7 candidate must be ${CANDIDATE_SHA}, received ${candidateSha}`,
  );
}
...
console.log("P11-F7 Secure Headers Zero-Unused Acceptance");
...
console.log("Zero-unused regression vs frozen pre-P11-F control");
console.log(
...
console.log(`\nP11-F7 ACCEPTANCE: ${accepted ? "PASS" : "FAIL"}`);
...
async function runWorker(args: readonly string[]): Promise<WorkerResult> {
  ...
  if (exitCode !== 0) {
    throw new Error(
      ["P11-F7 benchmark worker failed", ...args, stdout, stderr].join("\n"),
    );
  }
  ...
  if (last === undefined) {
    throw new Error("P11-F7 benchmark worker emitted no JSON");
  }

  const parsed: unknown = JSON.parse(last);
  if (!isWorkerResult(parsed)) {
    throw new Error(`Invalid P11-F7 worker result: ${last}`);
  }
...
function readOptions(values: readonly string[]): Options {
  ...
  if (controlRoot === undefined || controlRoot.length === 0) {
    throw new Error("P11-F7 benchmark requires --control-root=<path>");
  }

  if (candidateRoot === undefined || candidateRoot.length === 0) {
    throw new Error("P11-F7 benchmark requires --candidate-root=<path>");
  }
...
function assertCleanWorkingTree(root: string, label: string): void {
  ...
  if (status.length !== 0) {
    throw new Error(`P11-F7 ${label} worktree must be clean:\n${status}`);
  }
}

function median(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute median of empty P11-F7 sample set");
  }
...
function geometricMean(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute geometric mean of empty P11-F7 set");
  }
