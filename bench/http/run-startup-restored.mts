...
const variants = [
...

  {
    ...
    file: resolve(HERE, "servers/gelis-startup-restored.ts"),
  },
...
console.log("\nGelis startup-restored HTTP benchmark");
...
writeFileSync(
  resolve(RESULTS_DIR, "latest-startup-restored-http.json"),

  `${JSON.stringify(output, null, 2)}\n`,
);

console.log(
  "\nRaw results: " + "bench/http/results/latest-startup-restored-http.json",
);
...
function printComparison(rows: AdapterAggregateRow[]): void {
  console.log("\nStartup-restored throughput delta:");
  ...
  for (const benchmarkCase of cases) {
    ...
    const startupRestored = rows.find(
      (row) =>
        row.variant === "startup-restored" &&
        row.routeKind === benchmarkCase.routeKind &&
        row.bodyKind === benchmarkCase.bodyKind,
    );

    if (!plain || !startupRestored) {
      continue;
    }

    const delta =
      (startupRestored.requestsMedian / plain.requestsMedian - 1) * 100;
