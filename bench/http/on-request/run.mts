...
interface OnRequestHttpFramework {
  readonly name: string;
  readonly server: string;
  readonly env: Record<string, string>;
}

interface OnRequestHttpCase {
  readonly name: string;
...
] as const satisfies readonly OnRequestHttpFramework[];
...
] as const satisfies readonly OnRequestHttpCase[];
...
function createUrlFile(benchmarkCase: OnRequestHttpCase): string {
  const file = resolve(
    GENERATED_DIR,
...
function startServer(
  framework: OnRequestHttpFramework,
...
async function prewarmRoutes(benchmarkCase: OnRequestHttpCase): Promise<void> {
  const query = benchmarkCase.query ? "?page=42&q=gelis" : "";
