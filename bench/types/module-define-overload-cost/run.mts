...
const GENERATED_DIR = resolve(HERE, "../generated/module-define-overload-cost");
 ...
 * Diagnostic only.
 *
 * define-module-overloads uses the actual public defineModule surface.
 * narrow-static-signature exposes only the equivalent two-argument static
 * signature while preserving the same builder and ModuleRef result types.
 *
 * A material reduction in the narrow case isolates overload-resolution /
 * contextual-typing cost at module call sites.
 *
 * This does not replace or alter the frozen C8 gate.
 */
...
console.log("\nGelis P8-C8 defineModule overload-cost diagnostic");
...
for (const size of SIZES) {
  ...
  const overloaded = required("define-module-overloads", size);

  const narrow = required("narrow-static-signature", size);
  ...
  console.table(
    [
      ...
      ["define-module-overloads", overloaded],
      ["narrow-static-signature", narrow],
  ...
  console.log("narrow vs overloaded");

  console.table([
    {
      metric: "instantiations",

      ratio: round(narrow.instantiations / overloaded.instantiations, 3),

      delta: narrow.instantiations - overloaded.instantiations,
    },

    {
      metric: "memory",

      ratio: round(narrow.memoryMB / overloaded.memoryMB, 3),

      delta: round(narrow.memoryMB - overloaded.memoryMB, 1),
    },

    {
      metric: "check time",

      ratio: round(narrow.checkTime / overloaded.checkTime, 3),

      delta: round(narrow.checkTime - overloaded.checkTime, 3),
    },
  ]);
