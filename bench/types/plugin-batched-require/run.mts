...
const GENERATED_DIR = resolve(HERE, "../generated/plugin-batched-require");

const RESULTS_DIR = resolve(HERE, "../results/plugin-batched-require");
...
type Candidate = "batch-tuple-typed" | "batch-map-typed";

type Baseline = "batch-tuple-untyped" | "batch-map-untyped";

/*
 * P8-B2.3 acceptance gates.
 *
 * Frozen before results. Same thresholds as B2/B2.1/B2.2.
 *
 * Each candidate is compared with an untyped baseline using
 * the same API shape.
 *
 * At 5,000 plugins:
 ...
 *
 * If both pass and map is within 10% of tuple on absolute
 * 5k instantiations, memory, and check time, map wins the
 * ergonomic tie-break due named dependency destructuring.
 */
const GATES = {
...
const BASELINE_FOR: Record<Candidate, Baseline> = {
  "batch-tuple-typed": "batch-tuple-untyped",
  "batch-map-typed": "batch-map-untyped",
};
...
compile("individual-untyped", firstSize(SIZES));
...
function baselineForScenario(scenario: Scenario): Scenario {
  switch (scenario) {
    case "individual-generic-control":
      return "individual-untyped";

    case "batch-tuple-typed":
      return "batch-tuple-untyped";

    case "batch-map-typed":
      return "batch-map-untyped";
...
function recommend(
  ...
  const tupleGate = gateResults.find(
    (result) => result.scenario === "batch-tuple-typed",
  );

  const mapGate = gateResults.find(
    (result) => result.scenario === "batch-map-typed",
  );
  ...
  if (tupleGate?.pass && !mapGate?.pass) {
    return "batch-tuple-typed";
  }

  if (mapGate?.pass && !tupleGate?.pass) {
    return "batch-map-typed";
  }

  const tuple = requiredRow(rows, "batch-tuple-typed", 5000);

  const map = requiredRow(rows, "batch-map-typed", 5000);
...
function printMetadata(metadata: {
  ...
  console.log("\nGelis P8-B2.3 batched-require type benchmark");
...
function printGateTable(results: readonly GateResult[]): void {
  console.log("\nP8-B2.3 frozen acceptance gates");
