...
const GENERATED_DIR = resolve(HERE, "../generated/module-mount-cost");
 ...
 * Diagnostic only.
 *
 * mounted and unmounted cases are intentionally identical except for:
 *
 *   app.mount(moduleN)
 *
 * versus:
 ...
 *   void moduleN
 *
 * No tuple, array, union, or erasure is introduced in either case.
 * This isolates the compiler cost of the mount call itself.
 *
...
console.log("\nGelis P8-C8 module mount-cost diagnostic");
...
for (const size of SIZES) {
  ...
  const unmounted = required("module-exact-unmounted", size);

  const mounted = required("module-exact-mounted", size);
  ...
  console.table(
    [
      ...
      ["module-exact-unmounted", unmounted],
      ["module-exact-mounted", mounted],
  ...
  console.log("mounted vs unmounted");

  console.table([
    {
      metric: "instantiations",

      ratio: round(mounted.instantiations / unmounted.instantiations, 3),

      delta: mounted.instantiations - unmounted.instantiations,
    },

    {
      metric: "memory",

      ratio: round(mounted.memoryMB / unmounted.memoryMB, 3),

      delta: round(mounted.memoryMB - unmounted.memoryMB, 1),
    },

    {
      metric: "check time",

      ratio: round(mounted.checkTime / unmounted.checkTime, 3),

      delta: round(mounted.checkTime - unmounted.checkTime, 3),
    },
  ]);
