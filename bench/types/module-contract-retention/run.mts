...
const GENERATED_DIR = resolve(HERE, "../generated/module-contract-retention");
 ...
 *
 * This isolates:
 * - module definition cost vs mount cost
 * - runtime route registration without public named RouteRef retention
 * - exact named RouteRef retention
 * - explicitly erased named RouteRef retention
 *
 * It does not replace or modify the frozen C8 gate.
 */
...
console.log("\nGelis P8-C8 module contract-retention diagnostic");
...
for (const size of SIZES) {
  const baseline = required("direct-route-control", size);

  console.log(`${size.toLocaleString()} units`);

  console.table(
    rows
      .filter((row) => row.units === size)
      .map((row) => ({
        scenario: row.scenario,

        instantiations: row.diagnostics.instantiations,

        "inst x direct": round(
          row.diagnostics.instantiations / baseline.instantiations,
          3,
        ),

        "memory MB": round(row.diagnostics.memoryMB, 1),

        "mem x direct": round(row.diagnostics.memoryMB / baseline.memoryMB, 3),

        "check s": round(row.diagnostics.checkTime, 3),

        "check x direct": round(
          row.diagnostics.checkTime / baseline.checkTime,
          3,
        ),
      })),
  );
