...
const GENERATED_DIR = resolve(HERE, "../generated/plugin-model");

const RESULTS_DIR = resolve(HERE, "../results/plugin-model");
...
type StableCandidate = "token-tuple" | "token-map";

/*
 * P8-B2 acceptance gates.
 *
 * Frozen before candidate results:
 *
 * 1. Stable root is mandatory. root-generic is reference-only.
 * 2. Correctness probes must compile with every @ts-expect-error consumed.
 * 3. At 5,000 plugins a stable candidate must remain within:
 *      instantiations <= 1.50x baseline
 *      memory         <= 1.40x baseline
 *      check time     <= 1.60x baseline
 * 4. Candidate 100->5000 instantiation growth, normalized by baseline
 *    growth over the same range, must be <= 1.25x.
 * 5. If both stable models pass and token-map is within 10% of token-tuple
 *    for instantiations, memory, and check time at 5k, token-map wins the
 *    ergonomic tie-break because named injected dependencies are clearer
 *    than repeated setup-time require(token) calls.
 */
const GATES = {
...
const recommendation =
  gateResults === undefined
    ? undefined
    : recommendStableModel(rows, gateResults);
...
function evaluateGates(rows: readonly Row[]): GateResult[] {
  ...
  const candidates: readonly StableCandidate[] = ["token-tuple", "token-map"];
...
function recommendStableModel(
  rows: readonly Row[],
  gateResults: readonly GateResult[],
): StableCandidate | undefined {
  const tupleGate = gateResults.find(
    (result) => result.scenario === "token-tuple",
  );

  const mapGate = gateResults.find((result) => result.scenario === "token-map");
  ...
  const tuple = requiredRow(rows, "token-tuple", 5000);

  const map = requiredRow(rows, "token-map", 5000);

  const mapWithinErgonomicTie =
...
function printMetadata(metadata: {
  ...
  console.log("\nGelis P8-B2 plugin type-model benchmark");
