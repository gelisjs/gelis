...
const GENERATED_DIR = resolve(HERE, "../generated/module-return-retention");
 ...
 * Diagnostic only.
 *
 * exact-unmounted keeps defineModule()'s full inferred return type.
 * erased-unmounted immediately widens that return to AnyModuleRef.
 *
 * Both still execute the same defineModule() generic inference. Therefore a
 * material memory reduction in erased-unmounted points specifically at
 * retained return-type graph cost, not builder/inference work.
 *
 * This does not replace or modify the frozen C8 gate.
 */
...
console.log("\nGelis P8-C8 module return-retention diagnostic");
...
for (const size of SIZES) {
  ...
  const exact = required("module-exact-unmounted", size);

  const erased = required("module-erased-unmounted", size);
  ...
  console.table(
    [
      ...
      ["module-exact-unmounted", exact],
      ["module-erased-unmounted", erased],
  ...
  console.log("erased vs exact");

  console.table([
    {
      metric: "instantiations",

      ratio: round(erased.instantiations / exact.instantiations, 3),

      delta: erased.instantiations - exact.instantiations,
    },

    {
      metric: "memory",

      ratio: round(erased.memoryMB / exact.memoryMB, 3),

      delta: round(erased.memoryMB - exact.memoryMB, 1),
    },

    {
      metric: "check time",

      ratio: round(erased.checkTime / exact.checkTime, 3),

      delta: round(erased.checkTime - exact.checkTime, 3),
    },
  ]);
