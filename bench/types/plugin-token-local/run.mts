...
const GENERATED_DIR = resolve(HERE, "../generated/plugin-token-local");
const RESULTS_DIR = resolve(HERE, "../results/plugin-token-local");
...
type Candidate = "method-token-typed" | "callable-token-typed";
type Baseline = "method-token-untyped" | "callable-token-untyped";

/*
 * P8-B2.4 acceptance gates.
 *
 * Frozen before results. Thresholds are unchanged.
 * Each typed token candidate is compared with an untyped token
 * baseline using the identical public call shape.
 *
 * At 5,000 plugins:
 *   instantiations <= 1.50x matching baseline
 *   memory         <= 1.40x matching baseline
 *   check time     <= 1.60x matching baseline
 *
 * Normalized 100->5000 instantiation growth:
 *   <= 1.25x matching baseline growth
 *
 * If both pass and method-token-typed is within 10% of callable-token-typed
 * on absolute 5k instantiations, memory, and check time, method-token-typed
 * wins because Capability.require(context) / Capability.provide(context, v)
 * is more explicit than invoking a capability as a function.
 */
const GATES = {
...
const BASELINE_FOR: Record<Candidate, Baseline> = {
  "method-token-typed": "method-token-untyped",
  "callable-token-typed": "callable-token-untyped",
};
...
compile("method-token-untyped", firstSize(SIZES));
...
for (const size of SIZES) {
  for (const scenario of SCENARIOS) {
    ...
    rows.push({
      ...
      plugins: size,
      runs: RUNS,
      baseline: baselineForScenario(scenario),
...
for (const row of rows) {
  const baseline = requiredRow(rows, row.baseline, row.plugins);
...
writeFileSync(
  ...
  `${JSON.stringify(
    { metadata, rows, gateResults, recommendation },
    null,
    2,
  )}\n`,
);
...
function baselineForScenario(scenario: Scenario): Scenario {
  switch (scenario) {
    case "method-token-typed":
      return "method-token-untyped";
    case "callable-token-typed":
      return "callable-token-untyped";
    default:
      return scenario;
  }
}
...
function evaluateGates(rows: readonly Row[]): GateResult[] {
  ...
  return candidates.map((scenario) => {
    const baselineScenario = BASELINE_FOR[scenario];

    const baseline100 = requiredRow(rows, baselineScenario, 100);
    const baseline5000 = requiredRow(rows, baselineScenario, 5000);
    const candidate100 = requiredRow(rows, scenario, 100);
    const candidate5000 = requiredRow(rows, scenario, 5000);
    ...
    const checkVsBaseline5000 =
      candidate5000.checkTime / baseline5000.checkTime;
...
function recommend(
  ...
  const methodGate = gateResults.find(
    (result) => result.scenario === "method-token-typed",
  );
  const callableGate = gateResults.find(
    (result) => result.scenario === "callable-token-typed",
  );

  if (!methodGate?.pass && !callableGate?.pass) {
    return undefined;
  }

  if (methodGate?.pass && !callableGate?.pass) {
    return "method-token-typed";
  }

  if (callableGate?.pass && !methodGate?.pass) {
    return "callable-token-typed";
  }

  const method = requiredRow(rows, "method-token-typed", 5000);
  const callable = requiredRow(rows, "callable-token-typed", 5000);

  const methodWithinTie =
    method.instantiations <=
      callable.instantiations * GATES.ergonomicTieThreshold &&
    method.memoryMB <= callable.memoryMB * GATES.ergonomicTieThreshold &&
    method.checkTime <= callable.checkTime * GATES.ergonomicTieThreshold;

  if (methodWithinTie) {
    return "method-token-typed";
  }

  return callable.instantiations <= method.instantiations
    ? "callable-token-typed"
    : "method-token-typed";
}
...
function printMetadata(metadata: {
  ...
  console.log("\nGelis P8-B2.4 token-local type benchmark");
...
function printGateTable(results: readonly GateResult[]): void {
  console.log("\nP8-B2.4 frozen acceptance gates");
