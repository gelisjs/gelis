...
const GENERATED_DIR = resolve(HERE, "../generated/plugin-composition-scale");

const RESULTS_DIR = resolve(HERE, "../results/plugin-composition-scale");
...
/*
 * P8-B7 integrated production-plugin type gate.
 *
 ...
 *   equivalent routes, application scopes,
 *   request scopes, before/after hooks,
 *   and request/error hooks are registered
 *   directly on Gelis.
 *
 * plugin-composition:
 *   the same workload is declared through
 *   definePlugin + PluginSetupContext and
 *   installed through app.use().
 *
...
/*
 * Warm the compiler/process path once before
 * collecting medians.
 */
...
console.log("\nGelis P8-B7 plugin composition type scalability gate");
...
console.log("\nP8-B7 frozen integrated type gate");
...
console.log(
  `\nPre-frozen recommendation: ${gateResult.pass ? "accept-b7" : "reject-b7"}`,
);
...
function evaluateGate(allRows: readonly Row[]): {
  ...
  const candidate100 = requiredRow(allRows, "plugin-composition", 100);

  const candidate5000 = requiredRow(allRows, "plugin-composition", 5_000);
