...
const RESULTS_DIR = resolve(HERE, "../results/plugin-capability-constructor");
...
/*
 * P8-B3-A2 acceptance gate.
 *
 * Frozen before results.
 *
 * Candidate:
 *   new Capability<Value>("name")
 *
 * Baseline:
 *   preconstructed Capability<Value> token using the exact
 *   same generic class type and the same plugin use-site.
 *
 * generic-factory-control is diagnostic only.
 *
 * At 5,000 capabilities/plugins:
 *   instantiations <= 1.50x class-cast-control
 *   memory         <= 1.40x class-cast-control
 *   check time     <= 1.60x class-cast-control
 *
 * Normalized 100->5000 instantiation growth:
 *   <= 1.25x class-cast-control growth
 */
const GATES = {
...
compile("class-cast-control", firstSize(SIZES));
...
for (const size of SIZES) {
  ...
  const baseline = pending.find(
    (entry) => entry.scenario === "class-cast-control",
  )?.diagnostics;

  if (!baseline) {
    throw new Error(`Missing class-cast-control baseline for ${size}`);
  }
...
if (gateResult === undefined) {
  ...
  console.log("\nP8-B3-A2 frozen capability-constructor gate");

  console.table([
    {
      scenario: "class-constructor-typed",
...
function printMetadata(metadata: {
  ...
  console.log("\nGelis P8-B3-A2 capability-constructor benchmark");
...
function evaluateGate(rows: readonly Row[]) {
  const baseline100 = requiredRow(rows, "class-cast-control", 100);

  const baseline5000 = requiredRow(rows, "class-cast-control", 5000);

  const candidate100 = requiredRow(rows, "class-constructor-typed", 100);

  const candidate5000 = requiredRow(rows, "class-constructor-typed", 5000);
