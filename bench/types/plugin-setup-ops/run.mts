...
const GENERATED_DIR = resolve(HERE, "../generated/plugin-setup-ops");

const RESULTS_DIR = resolve(HERE, "../results/plugin-setup-ops");
...
type Candidate = "setup-generic-ops" | "token-bound-ops";

/*
 * P8-B2.2 acceptance gates.
 *
 * Frozen before results and intentionally unchanged from B2/B2.1.
 *
 * At 5,000 plugins:
 ...
 *
 * If both candidates pass and setup-generic-ops stays within 10%
 * of token-bound-ops on all 5k metrics, setup-generic-ops wins
 * the ergonomic tie-break because plugin.require(Capability) /
 * plugin.provide(Capability, value) is the clearer public API.
 */
const GATES = {
...
function recommend(
  ...
  const genericGate = gateResults.find(
    (result) => result.scenario === "setup-generic-ops",
  );

  const boundGate = gateResults.find(
    (result) => result.scenario === "token-bound-ops",
  );

  if (!genericGate?.pass && !boundGate?.pass) {
    return undefined;
  }

  if (genericGate?.pass && !boundGate?.pass) {
    return "setup-generic-ops";
  }

  if (boundGate?.pass && !genericGate?.pass) {
    return "token-bound-ops";
  }

  const generic = requiredRow(rows, "setup-generic-ops", 5000);

  const bound = requiredRow(rows, "token-bound-ops", 5000);

  const genericWithinTie =
    generic.instantiations <=
      bound.instantiations * GATES.ergonomicTieThreshold &&
    generic.memoryMB <= bound.memoryMB * GATES.ergonomicTieThreshold &&
    generic.checkTime <= bound.checkTime * GATES.ergonomicTieThreshold;

  if (genericWithinTie) {
    return "setup-generic-ops";
  }

  return bound.instantiations <= generic.instantiations
    ? "token-bound-ops"
    : "setup-generic-ops";
}
...
function printMetadata(metadata: {
  ...
  console.log("\nGelis P8-B2.2 setup-operation type benchmark");
...
function printGateTable(results: readonly GateResult[]): void {
  console.log("\nP8-B2.2 frozen acceptance gates");
