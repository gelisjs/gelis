...
const RESULTS_DIR = resolve(HERE, "../results/plugin-capability-contextual");
 ...
 *     defineCapability("database")
 *
 * defineCapability is non-generic and returns
 * Capability<never>. The explicit annotation binds the
 * value type and omission is fail-closed rather than any.
 *
 * Baseline:
 *   preconstructed Capability<Value> token with the same
 *   typed capability surface and plugin use-site.
 *
 * generic-factory-control is diagnostic only.
 *
 * At 5,000 capabilities/plugins:
 *   instantiations <= 1.50x typed-cast-control
 *   memory         <= 1.40x typed-cast-control
 *   check time     <= 1.60x typed-cast-control
 *
 * Normalized 100->5000 instantiation growth:
 *   <= 1.25x typed-cast-control growth
 */
const GATES = {
...
compile("typed-cast-control", firstSize(SIZES));
...
for (const size of SIZES) {
  ...
  const baseline = pending.find(
    (entry) => entry.scenario === "typed-cast-control",
  )?.diagnostics;

  if (!baseline) {
    throw new Error(`Missing typed-cast-control baseline for ${size}`);
  }
...
if (gateResult === undefined) {
  ...
  console.log("\nP8-B3-A3 frozen contextual-capability gate");

  console.table([
    {
      scenario: "contextual-factory-typed",
...
function printMetadata(metadata: {
  ...
  console.log("\nGelis P8-B3-A3 contextual capability benchmark");
...
function evaluateGate(rows: readonly Row[]) {
  const baseline100 = requiredRow(rows, "typed-cast-control", 100);

  const baseline5000 = requiredRow(rows, "typed-cast-control", 5000);

  const candidate100 = requiredRow(rows, "contextual-factory-typed", 100);

  const candidate5000 = requiredRow(rows, "contextual-factory-typed", 5000);
