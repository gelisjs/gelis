...
const GENERATED = new URL(
  "../generated/plugin-capability-factory/",
  import.meta.url,
);
...
function writeModel(
  ...
  const typed = scenario !== "factory-untyped";
  ...
  if (scenario === "factory-typed") {
    lines.push(
      "export function defineCapability<Value>(",
      "  name: string,",
      "): Capability<Value> {",
      "  return null as unknown as Capability<Value>",
      "}",
      "",
    );
  } else if (scenario === "factory-untyped") {
    lines.push(
      "export function defineCapability(",
      "  name: string,",
      "): Capability<any> {",
      "  return null as unknown as Capability<any>",
      "}",
      "",
    );
  }
  ...
  for (let index = 0; index < size; index++) {
    ...
    if (scenario === "factory-typed") {
      ...
      lines.push(
        `export const cap${index} = defineCapability("cap${index}")`,
        "",
      );
    } else {
      lines.push(
...
console.log(
  `Generated P8-B3-A capability-factory benchmarks for ${SIZES.join(", ")} capabilities/plugins`,
);
