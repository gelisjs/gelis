...
type Scenario = "routes" | "p11-g8-timeout-route";
...
for (const size of SIZES) generateTimeoutRoutes(size);
compile("routes", 100);

console.log("\nP11-G8 route-timeout TypeScript scalability acceptance\n");
console.log(`Runtime:       bun ${Bun.version}`);
console.log(`CPU:           ${cpus()[0]?.model ?? "unknown"}`);
console.log(`Runs/case:     ${RUNS}`);
console.log(`Sizes:         ${SIZES.join(", ")}`);
console.log("Control:       routes");
console.log("Candidate:     p11-g8-timeout-route");
...
for (const size of SIZES) {
  for (const scenario of ["routes", "p11-g8-timeout-route"] as const) {
    const samples: TypeDiagnostics[] = [];
    for (let run = 0; run < RUNS; run++) samples.push(compile(scenario, size));
...
const rows = SIZES.map((size) => {
  const candidate = findCase("p11-g8-timeout-route", size);
  ...
  return {
    routes: size,
    "inst ratio": round(instantiationsRatio, 4),
    "memory ratio": round(memoryRatio, 4),
    "check ratio": round(checkRatio, 4),
    "check gate": `<= ${checkLimit}x`,
    verdict: pass ? "PASS" : "FAIL",
  };
});

console.log("\nP11-G8 relative TypeScript summary\n");
console.table(rows);

const at1000 = findCase("p11-g8-timeout-route", 1000);
const at5000 = findCase("p11-g8-timeout-route", 5000);
...
console.log("\nP11-G8 1000→5000 growth summary\n");
console.table([
  {
    scenario: "p11-g8-timeout-route",
...
if (failed) throw new Error("P11-G8 TypeScript scalability gate failed");

console.log("\nStructural root-type assertion compiled successfully.");
console.log("P11-G8 TYPESCRIPT SCALABILITY: PASS");

function generateTimeoutRoutes(size: BenchmarkSize): void {
  const directory = caseDir("p11-g8-timeout-route", size);
  ...
  writeFileSync(
    ...
    [
      ...
      "export const routeDeadline = deadlines.route(1000)",
      "",
    ].join("\n"),
  );
  ...
  for (let start = 0, fileIndex = 0; start < size; start += ROUTES_PER_FILE, fileIndex++) {
    ...
    const lines = ["import { app, routeDeadline } from './app'", ""];

    for (let index = start; index < end; index++) {
      lines.push(
        `app.get('/bench/${index}/:id', { timeout: routeDeadline }, ` +
...
function parseDiagnostics(output: string): TypeDiagnostics {
  ...
  for (const line of output.split(/\r?\n/)) {
    ...
    if (key === "Instantiations") instantiations = parseNumber(raw);
    else if (key === "Memory used") memoryMB = parseMemoryMB(raw);
    else if (key === "Check time") checkTime = parseSeconds(raw);
    else if (key === "Total time") totalTime = parseSeconds(raw);
  }
...
function parseMemoryMB(value: string): number {
  ...
  if (trimmed.endsWith("K")) return Number(trimmed.slice(0, -1)) / 1024;
  if (trimmed.endsWith("M")) return Number(trimmed.slice(0, -1));
