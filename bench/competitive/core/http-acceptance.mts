...
const SERVERS = resolve(HERE, "servers");
...
console.log("Competitive Performance v0.1 — CP2-H real HTTP crown");
...
async function assertEnvironment(isProbeOnly: boolean): Promise<void> {
  if (Bun.version !== REQUIRED_BUN_VERSION) {
    throw new Error(
      `CP2-H requires Bun ${REQUIRED_BUN_VERSION}, received ${Bun.version}`,
    );
  }
  ...
  if (hono !== REQUIRED_HONO_VERSION) {
    throw new Error(`CP2-H Hono version mismatch: ${hono}`);
  }
  if (elysia !== REQUIRED_ELYSIA_VERSION) {
    throw new Error(`CP2-H Elysia stable version mismatch: ${elysia}`);
  }
  if (elysiaNext !== REQUIRED_ELYSIA_NEXT_VERSION) {
    throw new Error(`CP2-H Elysia next version mismatch: ${elysiaNext}`);
  }
  if (elysiaAot !== REQUIRED_ELYSIA_NEXT_VERSION) {
    throw new Error(`CP2-H Elysia AOT version mismatch: ${elysiaAot}`);
  }
  ...
  if (!isProbeOnly) {
    ...
    if (!ohaVersion.includes(REQUIRED_OHA_VERSION)) {
      throw new Error(
        `CP2-H requires oha ${REQUIRED_OHA_VERSION}, received ${ohaVersion}`,
      );
    }
  }
}
...
async function buildAotArtifacts(): Promise<void> {
  ...
  if (exitCode !== 0) {
    throw new Error(`CP2-H Elysia AOT build exited with ${exitCode}`);
  }
}
...
async function waitForServer(
  ...
  for (let attempt = 0; attempt < 400; attempt++) {
    if (server.exitCode !== null) {
      throw new Error(`CP2-H server exited early with ${server.exitCode}`);
    }
  ...
  throw new Error("CP2-H server did not become ready", { cause: lastError });
}
...
function assertCleanWorkingTree(): void {
  ...
  if (status.length !== 0) {
    throw new Error(`CP2-H worktree must be clean:\n${status}`);
  }
}
...
function assertProductionSourceIdentity(): void {
  ...
  if (result.exitCode === 1) {
    throw new Error(
      `CP2-H src/** differs from frozen production source ${CANDIDATE_SHA}`,
    );
  }
  throw new Error(new TextDecoder().decode(result.stderr));
}
...
function median(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute median of empty CP2-H set");
  }
