...
const WORKER = resolve(HERE, "p11-g11-timeout-worker.mts");
const HTTP_SERVER = resolve(HERE, "p11-g11-timeout-http-server.mts");
...
if (honoVersion !== HONO_VERSION) {
  throw new Error(
    `P11-G11 requires Hono ${HONO_VERSION}, received ${honoVersion}`,
  );
}
...
console.log("P11-G11 Timeout Hono + HTTP + Route-Scale Acceptance");
...
console.log(`\nP11-G11 ACCEPTANCE: ${accepted ? "PASS" : "FAIL"}`);
...
console.log();
console.log(
  "P11-G11 TIMEOUT HONO + HTTP + ROUTE-SCALE LOCAL ACCEPTANCE RUN: COMPLETE",
);

async function probeHttpFramework(
  framework: Framework,
  candidate: string,
): Promise<void> {
...
async function probeCombinedGelis(
  ...
  const coreUrl = new URL(
    `file://${resolve(candidate, "src/index.ts").replaceAll("\\", "/")}`,
  ).href;
  const requestIdUrl = new URL(
    `file://${resolve(candidate, "src/request-id/index.ts").replaceAll("\\", "/")}`,
  ).href;
  const timeoutUrl = new URL(
    `file://${resolve(candidate, "src/timeout/index.ts").replaceAll("\\", "/")}`,
  ).href;
  const [coreModule, requestIdModule, timeoutModule] = await Promise.all([
    ...
    import(requestIdUrl) as Promise<{
      requestId: (options?: Record<string, unknown>) => any;
    }>,
    import(timeoutUrl) as Promise<{
      timeout: (options?: Record<string, unknown>) => any;
    }>,
  ]);
  const app = new coreModule.Gelis();
  const ids = requestIdModule.requestId({ generator: () => "req_combined" });
  const deadlines = timeoutModule.timeout({
    duration: shouldTimeout ? 5 : 60_000,
  });
  app.use(ids);
  app.use(deadlines);
  app.get("/resource", () =>
  ...
  if (response.status !== expectedStatus) {
    throw new Error(
      `combined Gelis status ${response.status}, expected ${expectedStatus}`,
    );
  }
...
async function runDirectWorker(
  ...
  if (result.mode !== "direct") {
    throw new Error("P11-G11 direct worker returned wrong mode");
  }
...
async function runRouteScaleWorker(
  ...
  if (result.mode !== "route-scale") {
    throw new Error("P11-G11 route-scale worker returned wrong mode");
  }
...
async function runFireWorker(
  ...
  if (result.mode !== "fire") {
    throw new Error("P11-G11 fire worker returned wrong mode");
  }
...
async function waitForServer(
  ...
  throw new Error("P11-G11 HTTP server did not become ready", {
    cause: lastError,
  });
}
...
function assertCleanWorkingTree(root: string): void {
  ...
  if (status.length !== 0) {
    throw new Error(`P11-G11 candidate worktree must be clean:\n${status}`);
  }
}
...
function median(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute median of empty P11-G11 sample set");
  }
...
function geometricMean(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute geometric mean of empty P11-G11 set");
  }
