...
if (candidateSha !== CANDIDATE_SHA) {
  throw new Error(
    `P11-G9 candidate must be ${CANDIDATE_SHA}, received ${candidateSha}`,
  );
}
...
if (honoVersion !== HONO_VERSION) {
  throw new Error(
    `P11-G9 requires Hono ${HONO_VERSION}, received ${honoVersion}`,
  );
}
...
console.log("P11-G9 Request-ID Hono + HTTP Acceptance");
...
console.log(`\nP11-G9 ACCEPTANCE: ${accepted ? "PASS" : "FAIL"}`);
...
async function runWorker(args: readonly string[]): Promise<WorkerResult> {
  ...
  if (exitCode !== 0) {
    throw new Error(
      ["P11-G9 direct benchmark worker failed", ...args, stdout, stderr].join(
        "\n",
      ),
    );
  }
  ...
  if (last === undefined) {
    throw new Error("P11-G9 direct worker emitted no JSON");
  }

  const parsed: unknown = JSON.parse(last);
  if (!isWorkerResult(parsed)) {
    throw new Error(`Invalid P11-G9 worker result: ${last}`);
  }
...
async function waitForServer(
  ...
  throw new Error("P11-G9 HTTP server did not become ready", {
    cause: lastError,
  });
}
...
function readOptions(values: readonly string[]): Options {
  ...
  if (candidateRoot === undefined || candidateRoot.length === 0) {
    throw new Error("P11-G9 benchmark requires --candidate-root=<path>");
  }
...
function assertCleanWorkingTree(root: string): void {
  ...
  if (status.length !== 0) {
    throw new Error(`P11-G9 candidate worktree must be clean:\n${status}`);
  }
}
...
function median(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute median of empty P11-G9 sample set");
  }
...
function geometricMean(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute geometric mean of empty P11-G9 set");
  }
