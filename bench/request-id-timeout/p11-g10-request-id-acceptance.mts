...
const WORKER = resolve(HERE, "p11-g10-request-id-worker.mts");
const HTTP_SERVER = resolve(HERE, "p11-g10-request-id-http-server.mts");
...
console.log("P11-G10 Request-ID Hono + HTTP Acceptance");
...
if (probeOnly) {
  ...
  console.log(
    `Affinity:   logical CPU ${affinityLogicalCpu} (0x${affinityMaskHex}), HIGH direct workers\n`,
  );
}

let accepted = true;
const directScenarios: readonly RequestIdScenario[] = [
  "default-generated",
  "trusted-valid-inbound",
  "invalid-inbound-fallback",
  "custom-generator",
...
if (probeOnly) {
  ...
  console.log();
  console.log(
    "P11-G10 REQUEST-ID CORRECTNESS PROBE: PASS (10/10 framework checks)",
  );
  process.exit(0);
}

console.log("Request-ID direct comparison vs Hono 4.13.5");
console.log(
...
console.log(
  `\nRequest-ID direct geomean: ${formatRatio(directGeomean)} <= ${DIRECT_GEOMEAN_GATE.toFixed(2)}x => ${directGeomeanPass ? "PASS" : "FAIL"}\n`,
);

console.log("HTTP default generated request-ID comparison");
...
async function runPortableWorker(args: readonly string[]): Promise<string> {
  ...
  if (exitCode !== 0) {
    throw new Error(
      ["P11-G10 direct benchmark worker failed", ...args, stdout, stderr].join(
        "\n",
      ),
    );
  }
...
async function runPinnedWindowsWorker(
  ...
  if (exitCode !== 0) {
    throw new Error(
      ["P11-G10 pinned direct worker failed", ...args, stdout, stderr].join(
        "\n",
      ),
    );
  }
...
async function verifyHttpRequestId(
  url: string,
  framework: Framework,
): Promise<void> {
  ...
  const propagated = response.headers.get("x-request-id");
  const observed = response.headers.get("x-observed-request-id");

  if (
    propagated === null ||
    observed === null ||
    propagated !== observed ||
    !UUID_PATTERN.test(propagated)
  ) {
    throw new Error(`${framework} HTTP request-ID propagation mismatch`);
  }
}
...
function preflight(candidate: string): void {
  ...
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `P11-G10 requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }
  ...
  if (sourceDiff.exitCode !== 0) {
    throw new Error(
      "P11-G10 harness src/** must be byte-equivalent to frozen candidate",
    );
  }
  if (!probeOnly) {
    if (process.platform !== "win32") {
      throw new Error(
        "P11-G10 timed acceptance is authoritative only on Windows",
      );
    }
    if (logicalCpuCount !== EXPECTED_LOCAL_LOGICAL_CPUS) {
      throw new Error(
        `P11-G10 timed acceptance requires ${EXPECTED_LOCAL_LOGICAL_CPUS} logical CPUs, got ${logicalCpuCount}`,
      );
    }
  }
}
