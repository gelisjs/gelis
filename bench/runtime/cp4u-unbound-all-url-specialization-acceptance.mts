...
const CANDIDATE_SOURCE = "d00e9aa92bf36f59a4182d4602ca46392f5b3686";
const ROUTES = 5_000;
const SAMPLES = 11;
const HERE = dirname(fileURLToPath(import.meta.url));
const WORKER = join(HERE, "cp4u-unbound-all-url-specialization-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const CONTROL_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4u-control-${process.pid}-${Date.now()}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-U CORRECTNESS PROBE: PASS (${PAIRS.length * VARIANTS.length}/${PAIRS.length * VARIANTS.length})`,
  );
} else if (completed) {
  console.log();
  console.log("CP4-U LOCAL UNBOUND ALL URL SPECIALIZATION RUN: COMPLETE");
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-U unbound ALL URL specialization viability",
  );
...
function runTiming(): void {
  ...
  let passed = true;
  console.log();
  console.log("Frozen CP4-U unbound ALL URL specialization viability gates");
  ...
  console.log();
  console.log(
    `CP4-U UNBOUND ALL URL SPECIALIZATION VIABILITY GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-U requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-U requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-U requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(
      `src/** differs from frozen CP4-U source ${CANDIDATE_SOURCE}`,
    );
  }
  ...
  if (sourceAncestor.status !== 0) {
    throw new Error(
      `CP4-U source ${CANDIDATE_SOURCE} is not an ancestor of HEAD`,
    );
  }
}
...
function cleanupWorktree(): void {
  ...
  if (list.includes(basename(CONTROL_WORKTREE)) && cleanupError === undefined) {
    cleanupError = new Error("CP4-U control worktree remained after cleanup");
  }
