...
const appModule = (await import(
  `${pathToFileURL(appPath).href}?cp4z-app=${process.pid}-${Date.now()}`
...
const routerModule = (await import(
  `${pathToFileURL(routerPath).href}?cp4z-router=${process.pid}-${Date.now()}`
...
function staticOnlyCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(`http://gelis.test/s/${LAST}?source=cp4f`);
...
function mixedCell(
  ...
  if (target === "static") {
    const request = new Request(
      `http://gelis.test/s/${MIXED_LAST}?source=cp4f`,
    );
    return responseCell(app, request, "static", false);
  }

  const request = new Request(
    `http://gelis.test/d/${MIXED_LAST}/${PARAM_VALUE}?source=cp4f`,
  );
  const expected =
...
function mixedSameLengthCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(`http://gelis.test${requestPath}?source=cp4f`);
...
function trailingCell(
  ...
  const request = new Request(
    `http://gelis.test/r/${LAST}/${PARAM_VALUE}?source=cp4f`,
  );
  const expected =
    bodyKind === "json" ? JSON.stringify({ id: PARAM_VALUE }) : PARAM_VALUE;
  return responseCell(app, request, expected, bodyKind === "json");
...
function genericCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(
    `http://gelis.test/g/${LAST}/left/x/right?source=cp4f`,
  );
...
function collisionCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(
    `http://gelis.test${collisionRequestPath(LAST)}?source=cp4f`,
  );
  return responseCell(app, request, PARAM_VALUE, false);
}
...
function allCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(
    `http://gelis.test/a/${LAST}/${PARAM_VALUE}?source=cp4f`,
    { method: "PATCH" },
  );
  return responseCell(app, request, PARAM_VALUE, false);
}
...
function staticMemoryCell(Router: RouterConstructor): PreparedCell {
  ...
  return {
    ...
    singleMeasure: () => {
      ...
      if (delta <= 0) {
        throw new Error(
          `non-positive amplified static router heap delta: ${delta}`,
        );
      }
      return {
...
function requiredVariant(value: string | undefined): Variant {
  if (value === "production" || value === "candidate") {
    return value;
  }
