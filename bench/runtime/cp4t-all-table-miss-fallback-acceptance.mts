...
const CANDIDATE_SOURCE = "bc519e56c599d7b325f0db13d1c6d318801b6024";
const ROUTES = 5_000;
const SAMPLES = 11;
const HERE = dirname(fileURLToPath(import.meta.url));
const WORKER = join(HERE, "cp4t-all-table-miss-fallback-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const CONTROL_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4t-control-${process.pid}-${Date.now()}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-T CORRECTNESS PROBE: PASS (${PAIRS.length * VARIANTS.length}/${PAIRS.length * VARIANTS.length})`,
  );
} else if (completed) {
  console.log();
  console.log("CP4-T LOCAL ALL TABLE-MISS FALLBACK RUN: COMPLETE");
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-T ALL table-miss fallback viability",
  );
...
function runTiming(): void {
  ...
  let passed = true;
  console.log();
  console.log("Frozen CP4-T ALL table-miss fallback viability gates");
  ...
  console.log();
  console.log(
    `CP4-T ALL TABLE-MISS FALLBACK VIABILITY GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-T requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-T requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-T requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(
      `src/** differs from frozen CP4-T source ${CANDIDATE_SOURCE}`,
    );
  }
  ...
  if (sourceAncestor.status !== 0) {
    throw new Error(
      `CP4-T source ${CANDIDATE_SOURCE} is not an ancestor of HEAD`,
    );
  }
}
...
function cleanupWorktree(): void {
  ...
  if (list.includes(basename(CONTROL_WORKTREE)) && cleanupError === undefined) {
    cleanupError = new Error("CP4-T control worktree remained after cleanup");
  }
