...
const WORKER = join(HERE, "cp4ak-static-leading-mask-viability-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const RUN_TOKEN = `${process.pid}-${Date.now()}`;
const CONTROL_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4ak-control-${RUN_TOKEN}`,
);
const CANDIDATE_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4ak-candidate-${RUN_TOKEN}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-AK CORRECTNESS PROBE: PASS (${CELLS.length * SOURCES.length}/${CELLS.length * SOURCES.length})`,
  );
} else if (completed) {
  console.log();
  console.log("CP4-AK LOCAL STATIC-LEADING MASK VIABILITY RUN: COMPLETE");
}
...
function runTiming(): void {
  ...
  console.log();
  console.log("Frozen CP4-AK static-leading mask viability gates");
  ...
  console.log();
  console.log(
    `CP4-AK STATIC-LEADING MASK VIABILITY GATE: ${passed ? "PASS" : "FAIL"}`,
  );
  console.log(
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-AK requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AK requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AK requires a clean worktree:\n${dirty}`);
  }
