...
const weakScopes = new WeakMap<BaseContext, RequestScope>();
...
function beforeWeakMap(context: BaseContext): number {
  const scope = weakScopes.get(context);

  if (scope === undefined) {
    throw new Error("Missing WeakMap request scope");
  }

  return scope.userId + context.params.id.length;
}

function handlerWeakMap(context: BaseContext): number {
  const scope = weakScopes.get(context);
...
function afterWeakMap(context: BaseContext, result: number): number {
  const scope = weakScopes.get(context);
...
function runSymbolPostWrite(iterations: number): number {
  let local = 0;
  ...
  for (let index = 0; index < iterations; index++) {
    ...
    const context = createBaseContext(params) as BaseContext &
      Partial<SymbolContext>;

    Object.defineProperty(context, REQUEST_SCOPE, {
      configurable: true,
    ...
    const scopedContext = context as SymbolContext;

    local += beforeSymbol(scopedContext);

    const result = handlerSymbol(scopedContext);

    local += afterSymbol(scopedContext, result);
  }
...
function runWeakMap(iterations: number): number {
  let local = 0;
  ...
  for (let index = 0; index < iterations; index++) {
    ...
    weakScopes.set(context, scope);

    local += beforeWeakMap(context);

    const result = handlerWeakMap(context);

    local += afterWeakMap(context, result);
  }
...
const candidates = [
  {
    name: "symbol-post-write",

    run: runSymbolPostWrite,
  },
...
console.log("\nGelis P8-A5 request-capability runtime-shape diagnostic");
...
console.log(
  "Derivation:    identical pre-existing request-scope object lookup",
);
