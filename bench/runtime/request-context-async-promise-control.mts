...
/*
 * Shared Promise-based derivation.
 *
 * Both control and candidate call this exact function.
 * The control deliberately uses explicit .then()
 * composition instead of async/await so it does not
 * pay an avoidable async-function state machine.
 */
function deriveScope({
  ...
  return Promise.resolve(epoch).then(
    (currentEpoch) => scopePool[(currentEpoch + id.length) & 63]!,
  );
}
...
for (let index = 0; index < ROUTES; index++) {
  ...
  if (kind === 1) {
    ...
    /*
     * Manual Promise composition.
     *
     * No async function and no await here.
     */
    controlApp.get(
      path,

      (context) =>
        deriveScope(context).then((scope) => {
          numericSink ^=
  ...
  /*
   * Manual Promise-composed fused lower bound.
   *
   * Scope derivation is asynchronous, but the
   * before/handler/after work is fused into the
   * continuation without async/await syntax.
   */
  controlApp.get(
    path,
...
console.log(
  "\nGelis P8-A6.5b async request-context Promise-control confirmation",
);
...
console.log(
  "Derivation:    identical Promise-based function with one microtask continuation",
);

console.log(
  "Control:       ordinary route + explicit .then() manual scope delivery",
);
  ...
  return async (iterations) => {
    ...
    for (let index = 0; index < iterations; index++) {
      ...
      if (!(result instanceof Promise)) {
        throw new Error("Expected asynchronous P8-A6.5b route result");
      }
...
async function assertCorrectness(): Promise<void> {
  for (const request of [
    ...
    if (!(control instanceof Promise) || !(candidate instanceof Promise)) {
      throw new Error("Expected Promise from P8-A6.5b async correctness route");
    }
    ...
    if (controlResponse.status !== 204 || candidateResponse.status !== 204) {
      throw new Error("Unexpected P8-A6.5b response status");
    }
  }
}
