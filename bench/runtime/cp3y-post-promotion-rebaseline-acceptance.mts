...
const WORKER = join(HERE, "cp3y-post-promotion-rebaseline-worker.mts");
...
console.log(
  "Competitive Performance v0.1 — CP3-Y post-promotion residual rebaseline",
);
...
if (probeOnly) {
  ...
  console.log();
  console.log(
    `CP3-Y CORRECTNESS PROBE: PASS (${CELLS.length}/${CELLS.length})`,
  );
  process.exit(0);
}
...
console.log();
console.log("CP3-Y LOCAL POST-PROMOTION REBASELINE RUN: COMPLETE");
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP3-Y requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP3-Y requires a clean worktree:\n${dirty}`);
  }
