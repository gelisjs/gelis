 ...
 *    closure-bound request-local lifecycle.
 *
...
const registrationCases: readonly RegistrationCase[] = [
...

  {
    ...
    control: runDirectStaticLifecycleRegistration,
...

  {
    ...
    control: runDirectScopedLifecycleRegistration,
...
const directStaticLifecycleApp = new Gelis();

registerDirectStatic(directStaticLifecycleApp, true);

const moduleStaticLifecycleApp = new Gelis();

moduleStaticLifecycleApp.mount(createModuleStatic(true));

const directScopedLifecycleApp = new Gelis();

directScopedLifecycleApp.use(databasePlugin);

registerDirectScoped(directScopedLifecycleApp, true);
...
function runDirectStaticLifecycleRegistration(): void {
  const app = new Gelis();

  registerDirectStatic(app, true);
...
function runDirectScopedLifecycleRegistration(): void {
  const app = new Gelis();

  app.use(databasePlugin);

  registerDirectScoped(app, true);
