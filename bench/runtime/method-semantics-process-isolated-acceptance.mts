...
const CONTROL_SHA = "4466d0170cc97b94415a602093633e63c8de4034";
...
const workerPath = resolve(
  currentDirectory,
  "method-semantics-process-worker.mts",
);

console.log("\nP9-D6 process-isolated mirrored acceptance\n");
...
console.log(`Gate:           mirrored median delta <= +${MAX_DELTA_PERCENT}%`);
console.log(
  "Order buckets:  diagnostic only; removed as gates after identical A/A false-positive proof",
);

const results = [];

for (const workload of ["shared-response", "string-normalized"] as const) {
  results.push(await runWorkload(workload));
}

console.log("\nP9-D6 process-isolated mirrored acceptance summary\n");
...
if (failed.length !== 0) {
  throw new Error(
    `P9-D6 process-isolated zero-unused gate failed: ${failed
...
async function runWorkload(workload: Workload): Promise<{
   ...
   *   worker 4 = control
   *
   * Every worker imports exactly one Gelis module graph.
   */
  ...
  try {
    for (let sample = 0; sample < SAMPLES; sample++) {
      ...
      /*
       * Both orientation ratios are canonical
       * candidate / control.
       */
  ...
  const mirroredDeltas = samples.map((sample) => sample.deltaPercent);

  const controlStartDeltas = samples
    .filter((sample) => sample.order === "control-start")
    .map((sample) => sample.deltaPercent);

  const candidateStartDeltas = samples
    .filter((sample) => sample.order === "candidate-start")
    .map((sample) => sample.deltaPercent);

  const mirroredMedianDelta = median(mirroredDeltas);

  const controlStartMedianDelta = median(controlStartDeltas);

  const candidateStartMedianDelta = median(candidateStartDeltas);
