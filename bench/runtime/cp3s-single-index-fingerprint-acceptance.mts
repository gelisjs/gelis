...
const WORKER = join(HERE, "cp3s-single-index-fingerprint-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const BASELINE_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp3s-baseline-${process.pid}-${Date.now()}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP3-S CORRECTNESS PROBE: PASS (${PAIRS.length * 2}/${PAIRS.length * 2})`,
  );
} else if (completed) {
  console.log();
  console.log("CP3-S LOCAL PRODUCTION-SHAPE FINGERPRINT RUN: COMPLETE");
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP3-S single-index fingerprint candidate",
  );
...
function runTiming(): void {
  ...
  let passed = true;
  console.log();
  console.log("Frozen CP3-S candidate gates");
  ...
  console.log();
  console.log(
    `CP3-S PRODUCTION-SHAPE FINGERPRINT GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP3-S requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP3-S requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP3-S requires a clean worktree:\n${dirty}`);
  }
...
function cleanupBaselineWorktree(): void {
  ...
  if (remove.status !== 0) {
    throw new Error(
      `Failed to remove CP3-S baseline worktree:\n${remove.stderr}`,
    );
  }
  ...
  if (list.includes(BASELINE_WORKTREE)) {
    throw new Error("CP3-S baseline worktree remained after cleanup");
  }
}
