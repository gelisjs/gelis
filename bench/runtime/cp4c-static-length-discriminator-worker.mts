...
const appModule = (await import(
  `${pathToFileURL(appPath).href}?cp4c-app=${process.pid}-${Date.now()}`
...
const routerModule = (await import(
  `${pathToFileURL(routerPath).href}?cp4c-router=${process.pid}-${Date.now()}`
...
function staticOnlyCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(`http://gelis.test/s/${LAST}?source=cp4c`);
...
function mixedCell(
  ...
  if (target === "static") {
    const request = new Request(
      `http://gelis.test/s/${MIXED_LAST}?source=cp4c`,
    );
    return responseCell(app, request, "static", false);
  }

  const request = new Request(
    `http://gelis.test/d/${MIXED_LAST}/${PARAM_VALUE}?source=cp4c`,
  );
  const expected =
...
function mixedSameLengthCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(`http://gelis.test${requestPath}?source=cp4c`);
...
function trailingCell(
  ...
  const request = new Request(
    `http://gelis.test/r/${LAST}/${PARAM_VALUE}?source=cp4c`,
  );
  const expected =
    bodyKind === "json" ? JSON.stringify({ id: PARAM_VALUE }) : PARAM_VALUE;
  return responseCell(app, request, expected, bodyKind === "json");
...
function genericCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(
    `http://gelis.test/g/${LAST}/left/x/right?source=cp4c`,
  );
...
function requiredVariant(value: string | undefined): Variant {
  if (value === "production" || value === "cp4b" || value === "cp4c") {
    return value;
  }
...
function assertCell(value: string): asserts value is Cell {
  if (!CELLS.has(value as Cell)) {
    throw new Error(`Unknown CP4-C cell: ${value}`);
  }
}
