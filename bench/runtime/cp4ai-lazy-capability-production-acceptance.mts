...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-AI CORRECTNESS PROBE: PASS (${PAIRS.length * VARIANTS.length}/${PAIRS.length * VARIANTS.length})`,
  );
} else if (completed) {
  console.log();
  console.log(
...
function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-AI lazy request-URL capability direct production acceptance",
  );
...
function runTiming(): void {
  ...
  console.log();
  console.log(
    "Frozen CP4-AI lazy request-URL capability direct production acceptance gates",
  );
  ...
  console.log();
  console.log(
    `CP4-AI LAZY-CAPABILITY DIRECT PRODUCTION ACCEPTANCE GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-AI requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AI requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AI requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(
      `src/** differs from frozen CP4-AI source ${CANDIDATE_SOURCE}`,
    );
  }
  ...
  if (sourceNames !== "src/runtime/router.ts") {
    throw new Error(`Unexpected CP4-Z -> CP4-AI src/** delta:\n${sourceNames}`);
  }
}
...
function cleanupWorktree(): void {
  if (cp4zWorktreeCreated) {
    spawnSync(
      "git",
      ["-C", REPOSITORY_ROOT, "worktree", "remove", "--force", CP4_Z_WORKTREE],
      { encoding: "utf8" },
    );
  }
