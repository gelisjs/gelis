...
type Variant = "production" | "cp4b" | "cp4f" | "cp4i";
type Cell =
  | "static-only-raw"
  | "mixed-static-raw"
  | "mixed-dynamic-raw"
...
const ORDERS: readonly (readonly Variant[])[] = [
  ["production", "cp4b", "cp4f", "cp4i"],
  ["cp4b", "cp4i", "production", "cp4f"],
  ["cp4f", "production", "cp4i", "cp4b"],
  ["cp4i", "cp4f", "cp4b", "production"],
];
...
try {
  for (const source of SOURCES) {
    const path = resolve(
      REPOSITORY_ROOT,
      "..",
      `gelis-cp4m-${source.label}-${process.pid}-${Date.now()}`,
    );
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-M CORRECTNESS PROBE: PASS (${CELLS.length * SOURCES.length}/${CELLS.length * SOURCES.length})`,
  );
} else if (completed) {
  console.log();
  console.log("CP4-M LOCAL LINEAGE ATTRIBUTION RUN: COMPLETE");
}

function printHeader(): void {
  console.log("Competitive Performance v0.1 — CP4-M lineage attribution");
...
function runTiming(): void {
  ...
  console.log("| comparison | CP4-B | CP4-F | CP4-I |");
  console.log("| --- | ---: | ---: | ---: |");
  for (const spec of CELLS) {
    console.log(
      `| ${spec.label} | ${ratio(summaries, spec.cell, "cp4b", "production").toFixed(4)}x | ${ratio(summaries, spec.cell, "cp4f", "production").toFixed(4)}x | ${ratio(summaries, spec.cell, "cp4i", "production").toFixed(4)}x |`,
    );
  }
  ...
  for (const cell of ["static-only-raw", "mixed-static-raw"] as const) {
    console.log(
      `| ${cell} | ${ratio(summaries, cell, "cp4b", "production").toFixed(4)}x | ${ratio(summaries, cell, "cp4f", "cp4b").toFixed(4)}x | ${ratio(summaries, cell, "cp4i", "cp4f").toFixed(4)}x |`,
    );
  }
...
function firstCrossing(summaries: Map<string, Summary>, cell: Cell): string {
  const b = ratio(summaries, cell, "cp4b", "production");
  if (b > STATIC_BOUNDARY) return "production -> CP4-B";

  const f = ratio(summaries, cell, "cp4f", "production");
  if (f > STATIC_BOUNDARY) return "CP4-B -> CP4-F";

  const i = ratio(summaries, cell, "cp4i", "production");
  if (i > STATIC_BOUNDARY) return "CP4-F -> CP4-I";
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-M requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-M requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-M requires a clean worktree:\n${dirty}`);
  }
}
...
function cleanupWorktrees(): void {
  ...
  if (failures.length !== 0) {
    throw new Error(`CP4-M worktree cleanup failed:\n${failures.join("\n")}`);
  }
}
