...
const CANDIDATE_SOURCE = "8bb0327336ae970d73fc8638305a501205534ec2";
const ROUTES = 5_000;
const SAMPLES = 11;
const HERE = dirname(fileURLToPath(import.meta.url));
const WORKER = join(HERE, "cp4s-all-url-fallback-specialization-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const CONTROL_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4s-control-${process.pid}-${Date.now()}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-S CORRECTNESS PROBE: PASS (${PAIRS.length * VARIANTS.length}/${PAIRS.length * VARIANTS.length})`,
  );
} else if (completed) {
  console.log();
  console.log(
    "CP4-S LOCAL ALL REQUEST URL FALLBACK SPECIALIZATION RUN: COMPLETE",
  );
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-S ALL request URL fallback specialization viability",
  );
...
function runTiming(): void {
  ...
  const gates = [
    {
      label: "static-only guard",
      value: ratio(summaries, "static-only-raw"),
      limit: 1.02,
    },
    {
      label: "mixed-static recovery",
      value: ratio(summaries, "mixed-static-raw"),
      limit: 0.9963,
    },
    {
      label: "mixed dynamic raw guard",
      value: ratio(summaries, "mixed-dynamic-raw"),
      limit: 1.02,
    },
    {
      label: "mixed dynamic JSON guard",
      value: ratio(summaries, "mixed-dynamic-json"),
      limit: 1.02,
    },
    {
      label: "mixed same-length dynamic raw guard",
      value: ratio(summaries, "mixed-same-length-dynamic-raw"),
      limit: 1.02,
    },
    {
      label: "pure trailing dynamic raw guard",
      value: ratio(summaries, "trailing-dynamic-raw"),
      limit: 1.02,
    },
    {
      label: "pure trailing dynamic JSON guard",
      value: ratio(summaries, "trailing-dynamic-json"),
      limit: 1.02,
    },
    {
      label: "generic dynamic raw guard",
      value: ratio(summaries, "generic-dynamic-raw"),
      limit: 1.02,
    },
    {
      label: "forced collision raw guard",
      value: ratio(summaries, "collision-dynamic-raw"),
      limit: 1.02,
    },
    {
      label: "ALL dynamic raw guard",
      value: ratio(summaries, "all-dynamic-raw"),
      limit: 1.02,
    },
  ] as const;
  ...
  console.log();
  console.log(
    "Frozen CP4-S ALL request URL fallback specialization viability gates",
  );
  ...
  console.log();
  console.log(
    `CP4-S ALL REQUEST URL FALLBACK SPECIALIZATION VIABILITY GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-S requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-S requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-S requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(
      `src/** differs from frozen CP4-S source ${CANDIDATE_SOURCE}`,
    );
  }
  ...
  if (sourceAncestor.status !== 0) {
    throw new Error(
      `CP4-S source ${CANDIDATE_SOURCE} is not an ancestor of HEAD`,
    );
  }
}
...
function cleanupWorktree(): void {
  ...
  if (list.includes(basename(CONTROL_WORKTREE)) && cleanupError === undefined) {
    cleanupError = new Error("CP4-S control worktree remained after cleanup");
  }
