...
const WORKER = resolve(HERE, "cp4aq1e-memory-stability-calibration-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const RUN_TOKEN = `${process.pid}-${Date.now()}`;
const WORKTREE_A = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4aq1e-memory-a-${RUN_TOKEN}`,
);
const WORKTREE_B = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4aq1e-memory-b-${RUN_TOKEN}`,
);

type Source = "a" | "b";
type Order = "a-b" | "b-a";
type Unit = "ns/op" | "ms" | "bytes" | "bytes/router";
type Cell =
  | "static-only-raw"
  | "mixed-static-raw"
  | "mixed-dynamic-raw"
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-AQ1E-MEMORY CORRECTNESS PROBE: PASS (${CELLS.length * SOURCES.length}/${CELLS.length * SOURCES.length})`,
  );
} else if (completed) {
  console.log();
  console.log(
    "CP4-AQ1E-MEMORY LOCAL HARDENED STABILITY CALIBRATION RUN: COMPLETE",
  );
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-AQ1E-MEMORY hardened benchmark stability calibration",
  );
...
function printReadiness(diagnostics: ReadonlyMap<Cell, CellDiagnostics>): void {
  ...
  console.log();
  console.log("Frozen CP4-AQ1E-MEMORY benchmark-readiness criteria");
  console.log(
  ...
  console.log();
  console.log(
    `CP4-AQ1E-MEMORY HARDENED 5%-MEMORY-GATE READINESS: ${ready ? "PASS" : "FAIL"}`,
  );
  console.log(
    "AQ1E-MEMORY is environment/estimator calibration only; this readiness result does not reclassify Gelis source performance.",
  );
}
...
function spawnPinnedWindowsWorker(args: readonly string[]) {
  ...
  const resultFile = resolve(
    REPOSITORY_ROOT,
    "..",
    `gelis-cp4aq1e-memory-result-${process.pid}-${Date.now()}-${workerResultSequence++}.json`,
  );
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(
      `CP4-AQ1E-MEMORY requires Bun ${EXPECTED_BUN}, got ${Bun.version}`,
    );
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AQ1E-MEMORY requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AQ1E-MEMORY requires a clean worktree:\n${dirty}`);
  }

  if (!probeOnly) {
    if (process.platform !== "win32") {
      throw new Error(
        "CP4-AQ1E-MEMORY timed calibration is authoritative only on Windows",
      );
    }
    if (logicalCpuCount !== EXPECTED_LOCAL_LOGICAL_CPUS) {
      throw new Error(
        `CP4-AQ1E-MEMORY timed calibration requires ${EXPECTED_LOCAL_LOGICAL_CPUS} logical CPUs, got ${logicalCpuCount}`,
      );
    }
  }
