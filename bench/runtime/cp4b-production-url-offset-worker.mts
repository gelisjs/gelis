...
const appPath = required(args.appPath, "--app-path");
const probeOnly = args.probeOnly === "true";

const appModule = (await import(
  `${pathToFileURL(appPath).href}?cp4b=${process.pid}-${Date.now()}`
...
const prepared = prepareCell(Gelis, cell);
...
function mixedCell(
  ...
  if (target === "static") {
    const request = new Request(
      `http://gelis.test/s/${MIXED_LAST}?source=cp4b`,
    );
    return responseCell(app, request, "static", false);
  }

  const request = new Request(
    `http://gelis.test/d/${MIXED_LAST}/${PARAM_VALUE}?source=cp4b`,
  );
  const expected =
...
function trailingCell(
  ...
  const request = new Request(
    `http://gelis.test/r/${LAST}/${PARAM_VALUE}?source=cp4b`,
  );
  const expected =
    bodyKind === "json" ? JSON.stringify({ id: PARAM_VALUE }) : PARAM_VALUE;
  return responseCell(app, request, expected, bodyKind === "json");
...
function genericCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(
    `http://gelis.test/g/${LAST}/left/x/right?source=cp4b`,
  );
...
function collisionCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(
    `http://gelis.test${collisionRequestPath(LAST)}?source=cp4b`,
  );
...
function allCell(Gelis: GelisConstructor): PreparedCell {
  ...
  const request = new Request(
    `http://gelis.test/a/${LAST}/${PARAM_VALUE}?source=cp4b`,
    { method: "PATCH" },
  );
  return responseCell(app, request, PARAM_VALUE, false);
}
...
function assertCell(value: string): asserts value is Cell {
  if (!CELLS.has(value as Cell)) {
    throw new Error(`Unknown CP4-B cell: ${value}`);
  }
}
