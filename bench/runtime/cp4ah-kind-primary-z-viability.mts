...
const WORKER = join(HERE, "cp4ah-kind-primary-z-viability-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const RUN_TOKEN = `${process.pid}-${Date.now()}`;
const CP4Z_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4ah-cp4z-${RUN_TOKEN}`,
);
const CANDIDATE_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4ah-candidate-${RUN_TOKEN}`,
);

type Source = "cp4z" | "candidate";
...
const SOURCES: readonly Source[] = ["cp4z", "candidate"];
const ORDER_A: readonly Source[] = ["cp4z", "candidate"];
const ORDER_B: readonly Source[] = ["candidate", "cp4z"];
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-AH CORRECTNESS PROBE: PASS (${CELLS.length * SOURCES.length}/${CELLS.length * SOURCES.length})`,
  );
} else if (completed) {
  console.log();
  console.log("CP4-AH LOCAL KIND-PRIMARY Z VIABILITY RUN: COMPLETE");
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-AH Z/candidate KIND-primary viability",
  );
...
function runTiming(): void {
  ...
  console.log();
  console.log("Overall candidate / Z ratios");
  console.log("| comparison | candidate / Z |");
  console.log("| --- | ---: |");
  for (const spec of CELLS) {
    const cp4z = getSummary(summaries, spec.cell, "cp4z").median;
    const candidate = getSummary(summaries, spec.cell, "candidate").median;
    console.log(`| ${spec.label} | ${(candidate / cp4z).toFixed(4)}x |`);
  }

  console.log();
  console.log("Blockwise candidate / Z ratios");
  console.log(
  ...
  for (const spec of CELLS) {
    ...
    for (let block = 0; block < BLOCKS; block++) {
      ...
      ratios.push(candidateMedian / cp4zMedian);
    }
...
function runWorker(
  ...
  const sourceRoot = source === "cp4z" ? CP4Z_WORKTREE : CANDIDATE_WORKTREE;
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-AH requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AH requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AH requires a clean worktree:\n${dirty}`);
  }
  ...
  if (changed !== "src/runtime/router.ts") {
    throw new Error(
      `candidate must differ from CP4-Z only at src/runtime/router.ts; got:\n${changed}`,
    );
  }
}
