...
const PRODUCTION_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4ac-production-${RUN_TOKEN}`,
);
...
type Source = "production" | "cp4z" | "cp4ab";
...
const SOURCES: readonly Source[] = ["production", "cp4z", "cp4ab"];
const ORDERS: readonly (readonly Source[])[] = [
  ["production", "cp4z", "cp4ab"],
  ["production", "cp4ab", "cp4z"],
  ["cp4z", "production", "cp4ab"],
  ["cp4z", "cp4ab", "production"],
  ["cp4ab", "production", "cp4z"],
  ["cp4ab", "cp4z", "production"],
];
...
function printHeader(): void {
  ...
  console.log(`cp4ab      src: ${CP4AB_SOURCE}`);
...
function preflight(): void {
  ...
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AC requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AC requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(`src/** differs from frozen CP4-AB source ${CP4AB_SOURCE}`);
  }
}
