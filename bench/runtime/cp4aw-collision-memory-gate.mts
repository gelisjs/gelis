...
const CANDIDATE = "4e8ea606086da239e36bca994568968cbb778b2c";
const ROUTES = 5_000;
const BLOCKS = 8;
...
if (probeOnly) {
  console.log();
  console.log("CP4-AW COLLISION-MEMORY CORRECTNESS PROBE: PASS (2/2)");
} else if (completed && calibration) {
  console.log();
  console.log("CP4-AW COLLISION-MEMORY CALIBRATION RUN: COMPLETE");
} else if (completed) {
  console.log();
  console.log("CP4-AW COLLISION-MEMORY CANDIDATE RUN: COMPLETE");
}

function printHeader(): void {
  console.log("Competitive Performance v0.1 — CP4-AW collision-memory gate");
...
function printCalibrationDecision(value: Diagnostics): void {
  ...
  console.log();
  console.log("Frozen CP4-AW collision-memory calibration criteria");
  console.log(
  ...
  console.log();
  console.log(
    `CP4-AW COLLISION-MEMORY 5%-GATE READINESS: ${pass ? "PASS" : "FAIL"}`,
  );
}
...
function printCandidateDecision(value: Diagnostics): void {
  ...
  console.log();
  console.log("Frozen CP4-AW collision-memory candidate decision");
  ...
  console.log();
  console.log(`CP4-AW COLLISION-MEMORY DECISION: ${decision}`);
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(
      `CP4-AW memory gate requires Bun ${EXPECTED_BUN}, got ${Bun.version}`,
    );
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AW memory gate requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }
  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AW memory gate requires a clean worktree:\n${dirty}`);
  }
  if (!probeOnly) {
    if (process.platform !== "win32") {
      throw new Error(
        "CP4-AW timed memory runs are authoritative only on Windows",
      );
    }
    if (logicalCpuCount !== EXPECTED_LOCAL_LOGICAL_CPUS) {
      throw new Error(
        `CP4-AW timed memory runs require ${EXPECTED_LOCAL_LOGICAL_CPUS} logical CPUs, got ${logicalCpuCount}`,
      );
    }
  }
