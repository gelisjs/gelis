...
const CANDIDATE_SOURCE = "b268a99f58e6c055ed0255c30ea16783f85fb5f3";
const ROUTES = 5_000;
const SAMPLES = 11;
const HERE = dirname(fileURLToPath(import.meta.url));
const WORKER = join(HERE, "cp4q-deferred-upper-bound-dispatch-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const CONTROL_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4q-control-${process.pid}-${Date.now()}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-Q CORRECTNESS PROBE: PASS (${PAIRS.length * VARIANTS.length}/${PAIRS.length * VARIANTS.length})`,
  );
} else if (completed) {
  console.log();
  console.log("CP4-Q LOCAL DEFERRED UPPER-BOUND DISPATCH RUN: COMPLETE");
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-Q deferred upper-bound dispatch viability",
  );
...
function runTiming(): void {
  ...
  let passed = true;
  console.log();
  console.log("Frozen CP4-Q deferred upper-bound dispatch viability gates");
  ...
  console.log();
  console.log(
    `CP4-Q DEFERRED UPPER-BOUND DISPATCH VIABILITY GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-Q requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-Q requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-Q requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(
      `src/** differs from frozen CP4-Q source ${CANDIDATE_SOURCE}`,
    );
  }
  ...
  if (sourceAncestor.status !== 0) {
    throw new Error(
      `CP4-Q source ${CANDIDATE_SOURCE} is not an ancestor of HEAD`,
    );
  }
}
...
function cleanupWorktree(): void {
  ...
  if (list.includes(basename(CONTROL_WORKTREE)) && cleanupError === undefined) {
    cleanupError = new Error("CP4-Q control worktree remained after cleanup");
  }
