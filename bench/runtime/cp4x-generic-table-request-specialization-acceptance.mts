...
const SOURCES: readonly SourceSpec[] = [
  ...
  { label: "cp4x", sha: "efaa228231c920fee78053edeb5c8c094f324aca" },
];
...
try {
  for (const source of SOURCES) {
    const path = resolve(
      REPOSITORY_ROOT,
      "..",
      `gelis-cp4x-${source.label}-${process.pid}-${Date.now()}`,
    );
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-X CORRECTNESS PROBE: PASS (${CELLS.length * SOURCES.length}/${CELLS.length * SOURCES.length})`,
  );
} else if (completed) {
  console.log();
  console.log("CP4-X LOCAL BALANCED ACCEPTANCE RUN: COMPLETE");
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-X generic table request specialization balanced acceptance",
  );
...
function runTiming(): void {
  ...
  console.log("| comparison | CP4-U | CP4-X | W / U |");
  console.log("| --- | ---: | ---: | ---: |");
  for (const spec of CELLS) {
    console.log(
  ...
  let passed = true;
  console.log();
  console.log("Frozen CP4-X balanced acceptance gates");
  console.log("| gate | CP4-X / CP4-I | limit | result |");
  console.log("| --- | ---: | ---: | --- |");
  for (const spec of CELLS) {
    const value = ratio(summaries, spec.cell, "cp4x", "cp4i");
  ...
  console.log();
  console.log(
    `CP4-X GENERIC TABLE REQUEST SPECIALIZATION BALANCED ACCEPTANCE GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-X requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-X requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-X requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(
      `src/** differs from frozen CP4-X source ${SOURCES[2]!.sha}`,
    );
  }
}
...
function cleanupWorktrees(): void {
  ...
  if (failures.length !== 0) {
    throw new Error(`CP4-X worktree cleanup failed:\n${failures.join("\n")}`);
  }
}
