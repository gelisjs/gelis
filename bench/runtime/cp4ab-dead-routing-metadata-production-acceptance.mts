...
const CANDIDATE_SOURCE = "2114489c11d555edfc13db2a6336435fd4879931";
const ROUTES = 5_000;
const SAMPLES = 12;
const HERE = dirname(fileURLToPath(import.meta.url));
const WORKER = join(
  HERE,
  "cp4ab-dead-routing-metadata-production-acceptance-worker.mts",
);
const REPOSITORY_ROOT = resolve(HERE, "../..");
const PRODUCTION_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4ab-production-${process.pid}-${Date.now()}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-AB CORRECTNESS PROBE: PASS (${PAIRS.length * VARIANTS.length}/${PAIRS.length * VARIANTS.length})`,
  );
} else if (completed) {
  console.log();
  console.log(
    "CP4-AB LOCAL DEAD ROUTING METADATA PRODUCTION ACCEPTANCE RUN: COMPLETE",
  );
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-AB dead routing metadata direct production acceptance",
  );
...
function runTiming(): void {
  ...
  console.log();
  console.log(
    "Frozen CP4-AB dead routing metadata direct production acceptance gates",
  );
  ...
  console.log();
  console.log(
    `CP4-AB DEAD ROUTING METADATA DIRECT PRODUCTION ACCEPTANCE GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-AB requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AB requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AB requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(
      `src/** differs from frozen CP4-AB source ${CANDIDATE_SOURCE}`,
    );
  }
  ...
  if (sourceAncestor.status !== 0) {
    throw new Error(
      `CP4-AB source ${CANDIDATE_SOURCE} is not an ancestor of HEAD`,
    );
  }
}
...
function cleanupWorktree(): void {
  ...
  if (
    ...
    cleanupError = new Error(
      "CP4-AB production worktree remained after cleanup",
    );
  }
