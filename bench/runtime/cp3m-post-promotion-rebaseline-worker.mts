...
function pathnameCell(url: string, expectedPathname: string) {
  const request = new Request(url);

  return {
    operation: () => pathnameFromRequestUrl(request.url).length,
    assertCorrectness: () => {
      const pathname = pathnameFromRequestUrl(request.url);
      if (pathname !== expectedPathname) {
        throw new Error(
          `pathname mismatch: expected ${expectedPathname}, got ${pathname}`,
        );
      }
    },
  };
...
function assertCell(value: string): asserts value is Cell {
  if (!CELLS.has(value as Cell)) {
    throw new Error(`Unknown CP3-M cell: ${value}`);
  }
}
