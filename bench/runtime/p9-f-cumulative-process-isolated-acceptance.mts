...
const CONTROL_SHA = "a9740f2de5e8216b96de2b112d1aa79ae4b50dfe";
const SAMPLES = 41;
...
if (candidateDirty) {
  throw new Error("P9-F candidate worktree must be clean");
}

if (candidateHead === controlHead) {
  throw new Error("P9-F candidate equals cumulative control");
}

const here = dirname(fileURLToPath(import.meta.url));
const workerPath = resolve(here, "p9-f-cumulative-process-worker.mts");

console.log("\nP9-F-B cumulative runtime acceptance\n");
console.log(`Runtime:        bun ${Bun.version}`);
console.log(`CPU:            ${cpus()[0]?.model ?? "unknown"}`);
console.log(`Samples:        ${SAMPLES} mirrored samples/workload`);
console.log(`Control SHA:    ${controlHead}`);
console.log(`Candidate HEAD: ${candidateHead}`);
console.log("Candidate dirty:no");
console.log("Routes:         5,000/workload");
...
console.log("\nP9-F-B cumulative runtime summary\n");
...
if (failed.length !== 0) {
  throw new Error(
    `P9-F-B cumulative runtime gate failed: ${failed
...
async function createWorker(
  ...
  if (child.stdin === null || child.stdout === null) {
    throw new Error("Failed to create P9-F worker pipes");
  }
  ...
  const exitPromise = new Promise<void>((resolveExit, rejectExit) => {
    ...
    child.once("exit", (code) => {
      ...
      rejectExit(new Error(`P9-F worker exited with code ${code}`));
    });
  });
  ...
  if (ready.type !== "ready" || ready.workload !== workload) {
    throw new Error(`P9-F worker did not become ready for ${workload}`);
  }
  ...
  return {
    measure: async () => {
      if (closed) {
        throw new Error("P9-F worker is already closed");
      }
      ...
      if (message.type !== "measurement" || typeof message.ns !== "number") {
        throw new Error("Invalid P9-F worker measurement");
      }
...
function median(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute median of empty P9-F values");
  }
