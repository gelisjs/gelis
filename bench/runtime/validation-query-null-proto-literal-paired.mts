...
const CHILD = resolve(HERE, "validation-query-null-proto-literal-paired.mts");
...
interface PairSample {
  ...
  readonly order: "current-first" | "literal-first";
  readonly currentNs: number;
  readonly literalNs: number;
...
async function runParent(): Promise<void> {
  console.log(
    "Correctness: null-prototype literal parser matches current parser PASS\n",
  );

  console.log("Gelis P7-B3 null-prototype literal paired benchmark");
  ...
  console.log("Pairing:     current/literal same process");
  ...
  console.table(
    results.map((result) => ({
      ...
      "literal ns": round(result.literalMedianNs, 2),
      ...
      "literal CV %": round(result.literalCv * 100, 2),
      "literal faster": `${result.literalWins}/${SAMPLES}`,
    })),
  );
  ...
  console.table(
    results.map((result) => ({
      ...
      "literal-first Δ %": round(result.literalFirstMedianDeltaPercent, 2),
    })),
  );

  console.log(
    "\nInterpretation: negative delta favors null-prototype literal.",
  );
  console.log(
...
function runChild(scenario: Scenario): void {
  ...
  const literalOperation = createOperation(
    parseQueryWithNullProtoLiteral,
    urls,
  );

  warmup(currentOperation, WARMUP_ITERATIONS);
  warmup(literalOperation, WARMUP_ITERATIONS);

  const iterations = calibratePair(currentOperation, literalOperation);
  ...
  for (let sample = 0; sample < SAMPLES; sample++) {
    ...
    let literalElapsed: number;

    if (currentFirst) {
      currentElapsed = measure(currentOperation, iterations);
      literalElapsed = measure(literalOperation, iterations);
    } else {
      literalElapsed = measure(literalOperation, iterations);
      currentElapsed = measure(currentOperation, iterations);
    }
    ...
    const literalNs = millisecondsToNsPerOp(literalElapsed, iterations);

    const deltaNs = literalNs - currentNs;
    const deltaPercent = (literalNs / currentNs - 1) * 100;

    samples.push({
      sample: sample + 1,
      order: currentFirst ? "current-first" : "literal-first",
      currentNs,
      literalNs,
      deltaNs,
      deltaPercent,
  ...
  const literalValues = samples.map((sample) => sample.literalNs);
  ...
  const literalFirstDeltas = samples
    .filter((sample) => sample.order === "literal-first")
    .map((sample) => sample.deltaPercent);
  ...
  for (const sample of samples) {
    if (sample.literalNs < sample.currentNs) {
      literalWins++;
    } else if (sample.literalNs > sample.currentNs) {
      currentWins++;
    } else {
      ties++;
    }
  ...
  const result: ScenarioResult = {
    ...
    literalMedianNs: median(literalValues),
    ...
    literalCv: coefficientOfVariation(literalValues),
    literalWins,
    currentWins,
    ties,
    currentFirstMedianDeltaPercent: median(currentFirstDeltas),
    literalFirstMedianDeltaPercent: median(literalFirstDeltas),
    samples,
  };
...
function parseQueryWithNullProtoLiteral(url: string): QueryResult {
  const result = { __proto__: null } as unknown as QueryResult;
...
function verifyCorrectness(): void {
  ...
  for (const url of validUrls) {
    ...
    const literal = parseQueryWithNullProtoLiteral(url);

    assertEquivalent(current, literal, url);

    if (Object.getPrototypeOf(literal) !== null) {
  ...
  for (const url of invalidUrls) {
    ...
    const literalError = captureError(() =>
      parseQueryWithNullProtoLiteral(url),
    );

    if (
      currentError === undefined ||
      literalError === undefined ||
      currentError.constructor !== literalError.constructor
    ) {
...
function assertEquivalent(
  ...
  if (
    currentKeys.length !== literalKeys.length ||
    currentKeys.some((key, index) => key !== literalKeys[index])
  ) {
...
function calibratePair(current: () => void, literal: () => void): number {
  let iterations = 1000;

  while (true) {
    const currentElapsed = measure(current, iterations);
    const literalElapsed = measure(literal, iterations);

    const slowerElapsed = Math.max(currentElapsed, literalElapsed);
...
function isScenarioResult(value: unknown): value is ScenarioResult {
  ...
  return (
    ...
    typeof candidate.literalMedianNs === "number" &&
    ...
    typeof candidate.literalCv === "number" &&
    typeof candidate.literalWins === "number" &&
    typeof candidate.currentWins === "number" &&
    typeof candidate.ties === "number" &&
    typeof candidate.currentFirstMedianDeltaPercent === "number" &&
    typeof candidate.literalFirstMedianDeltaPercent === "number" &&
