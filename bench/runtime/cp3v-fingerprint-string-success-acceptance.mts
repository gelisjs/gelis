...
const CANDIDATE_SOURCE = "979821c709e809e29018791ce0fe212cded04162";
const ROUTES = 5_000;
const SAMPLES = 11;
const HERE = dirname(fileURLToPath(import.meta.url));
const WORKER = join(HERE, "cp3v-fingerprint-string-success-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const BASELINE_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp3v-baseline-${process.pid}-${Date.now()}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP3-V CORRECTNESS PROBE: PASS (${PAIRS.length * 2}/${PAIRS.length * 2})`,
  );
} else if (completed) {
  console.log();
  console.log("CP3-V LOCAL FINGERPRINT + STRING-SUCCESS RUN: COMPLETE");
}

function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP3-V fingerprint + string-success candidate",
  );
...
function runTiming(): void {
  ...
  let passed = true;
  console.log();
  console.log("Frozen CP3-V candidate gates (identical to CP3-S)");
  ...
  console.log();
  console.log(
    `CP3-V FINGERPRINT + STRING-SUCCESS GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function runWorker(
  ...
  const responsePath =
    variant === "production" ? BASELINE_RESPONSE : CANDIDATE_RESPONSE;
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP3-V requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP3-V requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP3-V requires a clean worktree:\n${dirty}`);
  }
...
function cleanupBaselineWorktree(): void {
  ...
  if (remove.status !== 0) {
    throw new Error(
      `Failed to remove CP3-V baseline worktree:\n${remove.stderr}`,
    );
  }
  ...
  if (list.includes(BASELINE_WORKTREE)) {
    throw new Error("CP3-V baseline worktree remained after cleanup");
  }
}
