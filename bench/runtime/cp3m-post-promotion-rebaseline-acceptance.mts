...
const WORKER = join(HERE, "cp3m-post-promotion-rebaseline-worker.mts");
...
console.log(
  "Competitive Performance v0.1 — CP3-M post-promotion residual rebaseline",
);
...
if (probeOnly) {
  ...
  console.log();
  console.log(
    `CP3-M CORRECTNESS PROBE: PASS (${CELLS.length}/${CELLS.length})`,
  );
  process.exit(0);
}
...
console.log();
console.log("CP3-M LOCAL POST-PROMOTION REBASELINE RUN: COMPLETE");
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP3-M requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP3-M requires a clean worktree:\n${dirty}`);
  }
