...
const CANDIDATE_SOURCE = "1b4057ad9d12bc28d2ab2ca7917924368fe99444";
const ROUTES = 5_000;
const SAMPLES = 12;
const HERE = dirname(fileURLToPath(import.meta.url));
const WORKER = join(
  HERE,
  "cp4z-mixed-static-first-production-acceptance-worker.mts",
);
const REPOSITORY_ROOT = resolve(HERE, "../..");
const PRODUCTION_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4z-production-${process.pid}-${Date.now()}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-Z CORRECTNESS PROBE: PASS (${PAIRS.length * VARIANTS.length}/${PAIRS.length * VARIANTS.length})`,
  );
} else if (completed) {
  console.log();
  console.log(
...
function printHeader(): void {
  console.log(
    "Competitive Performance v0.1 — CP4-Z mixed-static-first direct production acceptance",
  );
...
function runTiming(): void {
  ...
  console.log();
  console.log(
    "Frozen CP4-Z mixed-static-first direct production acceptance gates",
  );
  ...
  console.log();
  console.log(
    `CP4-Z MIXED-STATIC-FIRST DIRECT PRODUCTION ACCEPTANCE GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-Z requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-Z requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-Z requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(
      `src/** differs from frozen CP4-Z source ${CANDIDATE_SOURCE}`,
    );
  }
  ...
  if (sourceAncestor.status !== 0) {
    throw new Error(
      `CP4-Z source ${CANDIDATE_SOURCE} is not an ancestor of HEAD`,
    );
  }
}
...
function cleanupWorktree(): void {
  ...
  if (
    ...
    cleanupError = new Error(
      "CP4-Z production worktree remained after cleanup",
    );
  }
