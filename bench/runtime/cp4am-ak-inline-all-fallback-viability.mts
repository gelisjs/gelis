...
const WORKER = join(HERE, "cp4am-ak-inline-all-fallback-viability-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const RUN_TOKEN = `${process.pid}-${Date.now()}`;
const CONTROL_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4am-control-${RUN_TOKEN}`,
);
const CANDIDATE_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4am-candidate-${RUN_TOKEN}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-AM CORRECTNESS PROBE: PASS (${CELLS.length * SOURCES.length}/${CELLS.length * SOURCES.length})`,
  );
} else if (completed) {
  console.log();
  console.log("CP4-AM LOCAL AK INLINE ALL FALLBACK VIABILITY RUN: COMPLETE");
}
...
function printHeader(): void {
  ...
  console.log(`CP4-AK source:    ${CONTROL_SOURCE}`);
...
function runTiming(): void {
  ...
  console.log();
  console.log("Overall candidate / AK control ratios");
  console.log("| comparison | candidate / AK control |");
  console.log("| --- | ---: |");
  for (const spec of CELLS) {
  ...
  console.log();
  console.log("Frozen CP4-AM AK + inline ALL fallback viability gates");
  ...
  console.log();
  console.log(
    `CP4-AM AK INLINE ALL FALLBACK VIABILITY GATE: ${passed ? "PASS" : "FAIL"}`,
  );
  console.log(
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-AM requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AM requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AM requires a clean worktree:\n${dirty}`);
  }
