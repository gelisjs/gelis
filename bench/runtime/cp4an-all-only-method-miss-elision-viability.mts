...
const CONTROL_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4an-control-${RUN_TOKEN}`,
);
const CANDIDATE_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4an-candidate-${RUN_TOKEN}`,
);
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-AN CORRECTNESS PROBE: PASS (${CELLS.length * SOURCES.length}/${CELLS.length * SOURCES.length})`,
  );
} else if (completed) {
  console.log();
  console.log(
...
function runTiming(): void {
  ...
  console.log();
  console.log("Frozen CP4-AN ALL-only method-miss elision viability gates");
  ...
  console.log();
  console.log(
    `CP4-AN ALL-ONLY METHOD-MISS ELISION VIABILITY GATE: ${passed ? "PASS" : "FAIL"}`,
  );
  console.log(
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-AN requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AN requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AN requires a clean worktree:\n${dirty}`);
  }
