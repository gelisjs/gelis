...
type GlobalLifecyclePhase = "before" | "after";
...
type GlobalLifecycleCase = (typeof benchmarkCases)[number];

type GlobalLifecycleCaseName = GlobalLifecycleCase["name"];
...
function buildApp(scenario: GlobalLifecycleCaseName): Gelis {
  const app = new Gelis();
...
function configureEarlyGlobals(
  app: Gelis,
  scenario: GlobalLifecycleCaseName,
): void {
  switch (scenario) {
...
function benchmarkConfiguration(): RuntimeConfigurationRow[] {
  return [
...
function measureLateCompile(
  scenario: string,
  phase: GlobalLifecyclePhase,
...
function buildConfigurationApp(
  routes: number,
  phase: GlobalLifecyclePhase,
  existingHooks: number,
): Gelis {
...
function registerGlobalHook(app: Gelis, phase: GlobalLifecyclePhase): void {
  if (phase === "before") {
    app.onBeforeHandle(SYNC_BEFORE);
