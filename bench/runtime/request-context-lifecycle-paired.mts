...
for (let index = 0; index < ROUTES; index++) {
  ...
  const path = `/request-lifecycle/${index}/:id` as const;

  /*
   * Control = implementable fused lower bound.
   *
   * Derive once, then carry the local scope explicitly
   * through the three lifecycle phases inside one plain
   * route handler.
   */
  controlApp.get(path, (context) => {
  ...
  /*
   * Candidate = Gelis request-context lifecycle.
   *
   * The derivation and phase work are intentionally
   * equivalent to the fused control above.
   */
  requestRoutes.get(
    path,
...
const requests = {
  ...
  requestLifecycle: new Request("http://gelis.test/request-lifecycle/4999/abc"),
} as const;
...
const pairs: Pair[] = [
  ...
  {
    name: "request-scope-lifecycle",

    control: createRunner(controlApp, requests.requestLifecycle),

    candidate: createRunner(candidateApp, requests.requestLifecycle),
  },
];
...
console.log("\nGelis P8-A6.3 request-context lifecycle paired diagnostic");

console.log(`Runtime:       bun ${Bun.version}`);
console.log(`Routes/app:    ${ROUTES.toLocaleString()}`);
console.log(
  "Topology:      2500 plain + 1250 lifecycle + 1250 request-scope lifecycle",
);
console.log(`Samples:       ${SAMPLES}`);
console.log(`Iterations:    ${ITERATIONS.toLocaleString()} / side / sample`);
console.log(
  "Control:       fused derive + before + handler + after inside plain route",
);
  ...
  return (iterations) => {
    ...
    for (let index = 0; index < iterations; index++) {
      ...
      if (result instanceof Promise) {
        throw new Error(
          "Unexpected async result in synchronous P8-A6.3 benchmark",
        );
      }
...
function assertCorrectness(): void {
  for (const [controlRequest, candidateRequest] of [
    ...
    [requests.requestLifecycle, requests.requestLifecycle],
  ] as const) {
    ...
    if (control instanceof Promise || candidate instanceof Promise) {
      throw new Error(
        "Unexpected async result during P8-A6.3 correctness check",
      );
    }

    if (control.status !== 204 || candidate.status !== 204) {
      throw new Error("Unexpected P8-A6.3 response status");
    }
  }
}
