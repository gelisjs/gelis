...
const WORKER = join(HERE, "cp3f-residual-hotpath-worker.mts");
...
console.log("Residual production cells");
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP3-F requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP3-F requires a clean worktree:\n${dirty}`);
  }
