...
const WORKER = join(HERE, "cp4ao-all-only-production-acceptance-worker.mts");
const REPOSITORY_ROOT = resolve(HERE, "../..");
const PRODUCTION_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4ao-production-${process.pid}-${Date.now()}`,
);
const CP4_AK_WORKTREE = resolve(
  REPOSITORY_ROOT,
  "..",
  `gelis-cp4ao-cp4ak-${process.pid}-${Date.now()}`,
);
...
const ORDERS: readonly (readonly Variant[])[] = [
  ["production", "cp4ak", "candidate"],
  ["production", "candidate", "cp4ak"],
  ["cp4ak", "production", "candidate"],
  ["cp4ak", "candidate", "production"],
  ["candidate", "production", "cp4ak"],
  ["candidate", "cp4ak", "production"],
];
...
let cp4akWorktreeCreated = false;
let completed = false;
...
try {
  ...
  createWorktree(CP4_AK_WORKTREE, CP4_AK_SOURCE);
  cp4akWorktreeCreated = true;
...
if (probeCompleted) {
  console.log();
  console.log(
    `CP4-AO CORRECTNESS PROBE: PASS (${PAIRS.length * VARIANTS.length}/${PAIRS.length * VARIANTS.length})`,
  );
} else if (completed) {
  console.log();
  console.log("CP4-AO LOCAL ALL-ONLY PRODUCTION ACCEPTANCE RUN: COMPLETE");
}
...
function printHeader(): void {
  ...
  console.log(`CP4-AK source:   ${CP4_AK_SOURCE}`);
...
function runTiming(): void {
  ...
  console.log("Production / CP4-AK / candidate cells");
  ...
  for (const pair of PAIRS) {
    ...
    const cp4ak = getSummary(summaries, pair.cell, "cp4ak");
    const candidate = getSummary(summaries, pair.cell, "candidate");
    console.log(
      `| ${pair.label} | ${(cp4ak.median / production.median).toFixed(4)}x | ${(candidate.median / production.median).toFixed(4)}x | ${(candidate.median / cp4ak.median).toFixed(4)}x |`,
    );
  }
  ...
  console.log();
  console.log(
    `CP4-AO ALL-ONLY METHOD-MISS ELISION DIRECT PRODUCTION ACCEPTANCE GATE: ${passed ? "PASS" : "FAIL"}`,
  );
}
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-AO requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AO requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AO requires a clean worktree:\n${dirty}`);
  }
  ...
  if (sourceDiff.status !== 0) {
    throw new Error(
      `src/** differs from frozen CP4-AO source ${CANDIDATE_SOURCE}`,
    );
  }
  ...
  if (sourceAncestor.status !== 0) {
    throw new Error(
      `CP4-AO source ${CANDIDATE_SOURCE} is not an ancestor of HEAD`,
    );
  }
...
function cleanupWorktree(): void {
  if (cp4akWorktreeCreated) {
    spawnSync(
      "git",
      ["-C", REPOSITORY_ROOT, "worktree", "remove", "--force", CP4_AK_WORKTREE],
      { encoding: "utf8" },
    );
  }
