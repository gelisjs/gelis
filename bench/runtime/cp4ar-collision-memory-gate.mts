...
const CANDIDATE = "263185b8e83f1115392d5500d10e7509ef01c628";
const ROUTES = 5_000;
const BLOCKS = 8;
...
if (probeOnly) {
  console.log();
  console.log("CP4-AR COLLISION-MEMORY CORRECTNESS PROBE: PASS (2/2)");
} else if (completed && calibration) {
  console.log();
  console.log("CP4-AR COLLISION-MEMORY CALIBRATION RUN: COMPLETE");
} else if (completed) {
  console.log();
  console.log("CP4-AR COLLISION-MEMORY CANDIDATE RUN: COMPLETE");
}

function printHeader(): void {
  console.log("Competitive Performance v0.1 — CP4-AR collision-memory gate");
...
function printCandidateDecision(value: Diagnostics): void {
  ...
  console.log();
  console.log("Frozen CP4-AR collision-memory candidate decision");
...
function preflight(): void {
  ...
  if (Bun.revision !== EXPECTED_BUN_REVISION) {
    throw new Error(
      `CP4-AR memory gate requires Bun revision ${EXPECTED_BUN_REVISION}, got ${Bun.revision}`,
    );
  }
  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AR memory gate requires a clean worktree:\n${dirty}`);
  }
  if (!probeOnly) {
    ...
    if (logicalCpuCount !== EXPECTED_LOCAL_LOGICAL_CPUS) {
      throw new Error(
        `CP4-AR timed memory runs require ${EXPECTED_LOCAL_LOGICAL_CPUS} logical CPUs, got ${logicalCpuCount}`,
      );
    }
  }
