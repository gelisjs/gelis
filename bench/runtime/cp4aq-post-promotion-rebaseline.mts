...
const PRODUCTION_SOURCE = "5d9698d6b8d368ddcff358fc2645435b93c0c062";
const ROUTES = 5_000;
const SAMPLES = 11;
const HERE = dirname(fileURLToPath(import.meta.url));
const WORKER = join(HERE, "cp4aq-post-promotion-rebaseline-worker.mts");
...
console.log(
  "Competitive Performance v0.1 — CP4-AQ post-promotion residual rebaseline",
);
...
if (probeOnly) {
  ...
  console.log();
  console.log(
    `CP4-AQ CORRECTNESS PROBE: PASS (${CELLS.length}/${CELLS.length})`,
  );
  process.exit(0);
}
...
console.log("Post-promotion residual cells");
...
printRatio(
  "pathname dynamic/static request",
  "pathname-request-dynamic",
  "pathname-request-static",
);
printRatio(
...
console.log();
console.log("CP4-AQ LOCAL POST-PROMOTION REBASELINE RUN: COMPLETE");
...
function preflight(): void {
  if (Bun.version !== EXPECTED_BUN) {
    throw new Error(`CP4-AQ requires Bun ${EXPECTED_BUN}, got ${Bun.version}`);
  }

  const dirty = git(["status", "--porcelain"]);
  if (dirty !== "") {
    throw new Error(`CP4-AQ requires a clean worktree:\n${dirty}`);
  }
