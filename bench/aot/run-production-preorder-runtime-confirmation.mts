...
const WORKER = resolve(HERE, "production-preorder-runtime-worker.mts");
...
console.log(
  "\nGelis P6-E6-E4C.1 production preorder runtime stability confirmation",
);
...
const comparisons = KINDS.map((routeKind) => {
  ...
  const preorder = findRow(routeKind, "preorder");
  ...
  return {
    ...
    firstRatio: preorder.firstFetchMedian / flat.firstFetchMedian,
  };
});
...
const firstGeomean = geometricMean(
  comparisons.map((comparison) => comparison.firstRatio),
);

console.log("\nGeomean preorder vs flat\n");

console.log(`install ${signed((installGeomean - 1) * 100)}%`);

console.log(`ready   ${signed((readyGeomean - 1) * 100)}%`);

console.log(`first   ${signed((firstGeomean - 1) * 100)}%`);

console.log(`\nDecision: ${classify(comparisons, installGeomean)}`);
...
function classify(
  ...
  const generic = values.find((value) => value.routeKind === "generic");

  if (generic === undefined) {
    throw new Error("Missing generic benchmark comparison");
  }

  const staticAndTrailing = values.filter(
    (value) => value.routeKind !== "generic",
  );

  const strong =
    installGeomeanRatio <= 0.9 &&
    generic.installRatio <= 0.85 &&
    staticAndTrailing.every((value) => value.installRatio <= 1.03) &&
    values.every(
      (value) => value.readyRatio <= 1.03 && value.firstRatio <= 1.1,
    );
  ...
  const keep =
    installGeomeanRatio <= 0.95 &&
    generic.installRatio <= 0.9 &&
    staticAndTrailing.every((value) => value.installRatio <= 1.05) &&
    values.every(
      (value) => value.readyRatio <= 1.05 && value.firstRatio <= 1.15,
    );
  ...
  const reject =
    ...
    generic.installRatio >= 0.95 ||
    values.some(
      (value) =>
        value.installRatio > 1.1 ||
...
async function runWorker(
  ...
  if (exit !== 0) {
    throw new Error(
      [
        "Production preorder runtime worker failed",
