...
import { compilePreorderAotArtifactCandidate } from "./preorder-aot-candidate.mts";

const HERE = dirname(fileURLToPath(import.meta.url));

const ROOT = resolve(HERE, "../..");

const WORKER = resolve(HERE, "preorder-serialized-runtime-worker.mts");

const TMP = resolve(HERE, "tmp", "preorder-serialized-runtime");
...
const SCENARIOS = ["current-flat", "preorder"] as const;
...
for (const shapeKind of SHAPES) {
  ...
  const preorder = compilePreorderAotArtifactCandidate(plan, flat);

  const currentFlatPath = resolve(TMP, `${shapeKind}-flat.json`);

  const preorderPath = resolve(TMP, `${shapeKind}-preorder.json`);

  await Bun.write(currentFlatPath, JSON.stringify(flat));

  await Bun.write(preorderPath, JSON.stringify(preorder));
...
console.log("\nGelis P6-E6-E3D.4D preorder topology serialized runtime");
...
for (const shapeKind of SHAPES) {
  ...
  const preorder = findRow(shapeKind, "preorder");

  const currentLoadParse = current.loadMs + current.parseMs;

  const preorderLoadParse = preorder.loadMs + preorder.parseMs;

  const ratio = {
    shapeKind,

    readyRatio: preorder.readyMs / current.readyMs,

    loadParseRatio: preorderLoadParse / currentLoadParse,

    hydrateRatio: preorder.hydrateMs / current.hydrateMs,

    firstRatio: preorder.firstFetchUs / current.firstFetchUs,
  };
...
console.log("\nGeomean preorder vs current flat");
...
console.log(
  `\nDecision: ${classify(ratios, readyGeo, loadParseGeo, hydrateGeo)}`,
);
...
function classify(
  ...
  if (
    ...
    loadParseGeo <= 1 &&
    ratios.every(
      (value) => value.readyRatio <= 1.03 && value.firstRatio <= 1.15,
    )
  ) {
    return "ADVANCE";
  }
...
async function runWorker(
  ...
  const artifactPath =
    scenario === "current-flat" ? info.currentFlatPath : info.preorderPath;
  ...
  if (exit !== 0) {
    throw new Error(
      [
        "Serialized preorder runtime worker failed",
