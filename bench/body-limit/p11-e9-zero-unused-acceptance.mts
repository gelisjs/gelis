...
const WORKER = resolve(HERE, "p11-e9-zero-unused-worker.mts");

const CONTROL_SHA = "3628c82d1d38539c24a6f88ec2c6717ec8c3542f";
const CANDIDATE_SHA = "be6872b38642ddf06538ab6eef7d11054f02f587";
...
if (controlSha !== CONTROL_SHA) {
  throw new Error(
    `P11-E9 control must be ${CONTROL_SHA}, received ${controlSha}`,
  );
}

if (candidateSha !== CANDIDATE_SHA) {
  throw new Error(
    `P11-E9 candidate must be ${CANDIDATE_SHA}, received ${candidateSha}`,
  );
}
...
console.log("P11-E9 Body Limit Zero-Unused Acceptance");
...
console.log("Zero-unused regression vs frozen pre-P11-E control");
console.log(
...
console.log(`\nP11-E9 ACCEPTANCE: ${accepted ? "PASS" : "FAIL"}`);
...
async function runWorker(args: readonly string[]): Promise<WorkerResult> {
  ...
  if (exitCode !== 0) {
    throw new Error(
      ["P11-E9 benchmark worker failed", ...args, stdout, stderr].join("\n"),
    );
  }
  ...
  if (last === undefined) {
    throw new Error("P11-E9 benchmark worker emitted no JSON");
  }

  const parsed: unknown = JSON.parse(last);
  if (!isWorkerResult(parsed)) {
    throw new Error(`Invalid P11-E9 worker result: ${last}`);
  }
...
function readOptions(values: readonly string[]): Options {
  ...
  if (controlRoot === undefined || controlRoot.length === 0) {
    throw new Error("P11-E9 benchmark requires --control-root=<path>");
  }

  if (candidateRoot === undefined || candidateRoot.length === 0) {
    throw new Error("P11-E9 benchmark requires --candidate-root=<path>");
  }
...
function assertCleanWorkingTree(root: string, label: string): void {
  ...
  if (status.length !== 0) {
    throw new Error(`P11-E9 ${label} worktree must be clean:\n${status}`);
  }
}

function median(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute median of empty P11-E9 sample set");
  }
...
function geometricMean(values: readonly number[]): number {
  if (values.length === 0) {
    throw new Error("Cannot compute geometric mean of empty P11-E9 set");
  }
