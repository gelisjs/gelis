...
CANDIDATE = "bc519e56c599d7b325f0db13d1c6d318801b6024"

source = Path("bench/runtime/cp4s-all-url-fallback-specialization-acceptance.mts").read_text()
worker = Path("bench/runtime/cp4s-all-url-fallback-specialization-worker.mts").read_text()

source = source.replace(
    "cp4s-all-url-fallback-specialization-worker.mts",
    "cp4t-all-table-miss-fallback-worker.mts",
)
source = source.replace("gelis-cp4s-control-", "gelis-cp4t-control-")
source = source.replace(
    "8bb0327336ae970d73fc8638305a501205534ec2",
    CANDIDATE,
)
source = source.replace("CP4-S", "CP4-T")
source = source.replace(
    "ALL request URL fallback specialization viability",
    "ALL table-miss fallback viability",
)
source = source.replace(
    "ALL REQUEST URL FALLBACK SPECIALIZATION VIABILITY",
    "ALL TABLE-MISS FALLBACK VIABILITY",
...
source = source.replace(
    "CP4-T LOCAL ALL REQUEST URL FALLBACK SPECIALIZATION RUN: COMPLETE",
    "CP4-T LOCAL ALL TABLE-MISS FALLBACK RUN: COMPLETE",
)

worker = worker.replace("cp4s-app=", "cp4t-app=")
worker = worker.replace("cp4s-router=", "cp4t-router=")

Path("bench/runtime/cp4t-all-table-miss-fallback-acceptance.mts").write_text(source)
Path("bench/runtime/cp4t-all-table-miss-fallback-worker.mts").write_text(worker)

freeze = f'''# Competitive Performance v0.1 — CP4-T ALL table-miss fallback viability freeze
...
Path("docs/benchmarks/competitive-cp4t-all-table-miss-fallback-freeze.md").write_text(freeze)
