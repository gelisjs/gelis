...
CANDIDATE = "8bb0327336ae970d73fc8638305a501205534ec2"

acceptance = Path("bench/runtime/cp4r-upper-bound-sentinel-dispatch-acceptance.mts").read_text()
...
acceptance = acceptance.replace("gelis-cp4r-control-", "gelis-cp4s-control-")
acceptance = acceptance.replace(
    "49953b7ecb366c8c300dd2968ce2d8656b41a315",
    CANDIDATE,
)
acceptance = acceptance.replace("CP4-R", "CP4-S")
acceptance = acceptance.replace(
...
acceptance = acceptance.replace(
    "CP4-S LOCAL UPPER-BOUND SENTINEL DISPATCH RUN: COMPLETE",
    "CP4-S LOCAL ALL REQUEST URL FALLBACK SPECIALIZATION RUN: COMPLETE",
)

Path("bench/runtime/cp4s-all-url-fallback-specialization-acceptance.mts").write_text(
    acceptance
)
Path("bench/runtime/cp4s-all-url-fallback-specialization-worker.mts").write_text(
    'import "./cp4p-mixed-positive-packed-state-worker.mts";\n'
)

freeze = f'''# Competitive Performance v0.1 — CP4-S ALL request URL fallback specialization viability freeze
...
'''
Path("docs/benchmarks/competitive-cp4s-all-url-fallback-specialization-freeze.md").write_text(
    freeze
)
