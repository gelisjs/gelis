...
CP4J_REF = "origin/work/competitive-cp4j-full-production-acceptance"

def git_show(path: str) -> str:
    return subprocess.check_output(
        ["git", "show", f"{CP4J_REF}:{path}"],
        text=True,
    )

src = git_show("bench/runtime/cp4j-full-production-acceptance.mts")
...
worker = git_show("bench/runtime/cp4j-full-production-worker.mts")
worker = worker.replace('cp4j', 'cp4y')
worker = worker.replace('CP4-J', 'CP4-Y')
Path('bench/runtime/cp4y-direct-production-revalidation-worker.mts').write_text(worker)
