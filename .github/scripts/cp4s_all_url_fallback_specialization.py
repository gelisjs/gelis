...
path = Path("src/runtime/router-all.ts")
text = path.read_text()

old = '''export function activateAllFallback(router: Router): void {
...
new = '''export function activateAllFallback(router: Router): void {
  if (!Object.prototype.hasOwnProperty.call(router, "match")) {
    const exactMatch = router.match.bind(router);
...
if old not in text:
    raise SystemExit("CP4-S router-all anchor not found")
